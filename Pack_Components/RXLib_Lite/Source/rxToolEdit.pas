...
uses
  ...
  Dialogs,FileCtrl, Mask, rxDateUtil;
...
type
  //-- EAxialDateError
 EAxialDateError = class(Exception)
 end;
...
{$ENDIF}
    ...
    FDateFormat: string{$IFNDEF RX_D14}[10]{$ENDIF};
    FFormatting: Boolean;
...
uses
  ShellAPI, Consts {$IFDEF RX_D3} ,ExtDlgs {$ENDIF} ,RxResConst
  {$IFDEF RX_D6} ,Variants {$ENDIF}
  ,rxMaxMin, rxPickDate, rxStrUtils;
...
//      PopupCloseUp(FPopup, Key = Char(VK_RETURN));
  ...
  if {$IFDEF UNICODE}CharInSet(Key, [#10, #9]){$ELSE}Key in [#10, #9]{$ENDIF} then begin
    Key := #0;
...
begin
  if TextStored then
    try
      ...
      begin
        if CanFocus then SetFocus;
        raise EAxialDateError.Create(E.Message);
      end;
    end;
end;
...
var
  DateValue: TDateTime;
  OldFormat: string{$IFNDEF RX_D14}[10]{$ENDIF};
begin
  DateValue := GetDate;
...
begin
  if CharInSet(Key, ['T', 't', '+', '-']) and PopupVisible then begin
...
begin
  ...
  if CalendarStyle = csDialog then begin
    D := Self.Date;
    Action := SelectDate(Self, D, DialogTitle, FStartOfWeek, FWeekends,
...
begin
  if not FDateAutoBetween then
    if Value <> NullDate then begin
      ...
      then raise Exception.CreateFmt(RxLoadStr(SDateOutOfRange),[FormatDateTime(FDateFormat, Value),FormatDateTime(FDateFormat, FMinDate), FormatDateTime(FDateFormat, FMaxDate)])
      else if ((FMinDate <> NullDate) and (Value < FMinDate))
        then raise Exception.CreateFmt(RxLoadStr(SDateOutOfMin),[FormatDateTime(FDateFormat, Value),FormatDateTime(FDateFormat,FMinDate)])
        else if ((FMaxDate <> NullDate) and (Value > FMaxDate))
          then raise Exception.CreateFmt(RxLoadStr(SDateOutOfMax),[FormatDateTime(FDateFormat, Value),FormatDateTime(FDateFormat,FMaxDate)]);
    end;
