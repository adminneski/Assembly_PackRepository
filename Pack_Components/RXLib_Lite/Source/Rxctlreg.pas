...
{$ENDIF DCS}
  ...
  rxToolEdit, rxDateUtil,
  rxPickDate, Consts, RxResConst;
//  rxVCLUtils,
//  {$IFDEF USE_RX_GIF}  {$ENDIF};
  {rxFileUtil, Rxsplit;}
...
{$ENDIF}
    procedure EditImage(Image: TImage);
    procedure LoadAniFile(Image: TImage);
  public
...
procedure TAnimatedEditor.EditImage(Image: TImage);
var
{$IFDEF RX_D6}  // Polaris
...
procedure TAnimatedEditor.LoadAniFile(Image: TImage);
var
  Dialog: TOpenDialog;
  AniCursor: TImage;
  CurDir: string;
  ...
  try
    with Dialog do begin
      ...
      if Execute then begin
        AniCursor := TImage.Create;
        try
          AniCursor.Picture.LoadFromFile(FileName);
...
begin
  if (Index = GetVerbCount - 1) then
    LoadAniFile(TImage(Component))
  else if (Index = GetVerbCount - 2) then
    EditImage(TImage(Component))
...
{$ENDIF}
begin
  RegisterComponents('RXLib Lite - Controls', [// TComboEdit,
                                               TDateEdit]);
    {$IFDEF USE_RX_GIF}  {$ENDIF}
...
{$IFDEF RX_D3}
  RegisterNonActiveX([TCustomComboEdit, TCustomDateEdit, TCustomEdit,
  TCustomListBox], axrComponentOnly);
  RegisterNonActiveX([TScroller], axrComponentOnly);
{$ENDIF RX_D3}

  RegisterPropertyEditor(TypeInfo(TDayOfWeekName), nil, '', TWeekDayProperty);
{$IFDEF RX_D3}
  RegisterPropertyEditor(TypeInfo(string), TCustomEdit, 'Text', nil);
{$ELSE}
