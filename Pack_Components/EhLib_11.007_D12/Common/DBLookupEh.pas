...
begin
  if ListActive and not InrenalListDataChanging and
    not FDataList.InrenalListDataChanging then
  begin
  end;
end;
...
function TCustomDBLookupComboboxEh.InrenalListDataChanging;
begin
  Result := (FInrenalListDataChanging > 0);
end;

procedure TCustomDBLookupComboboxEh.BeginInrenalListDataChanging;
begin
  Inc(FInrenalListDataChanging);
end;

procedure TCustomDBLookupComboboxEh.EndInrenalListDataChanging;
begin
...
begin
  inherited;
  if (Style = csDropDownEh) and
     not SpecialKey and
     not (Message.CharCode = 0) then
  begin
    ...
    begin
      ...
      if (Message.CharCode <> VK_BACK) and LocateStr(Text, False) then
      begin
        SelStart := Length(Text);
