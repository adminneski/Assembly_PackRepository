...
type
  ...
  public
    ...
    function GetDataSetLikeWildcardForSeveralCharacters: String; override;
    function GetLocalFilterApplyingWay: TLocalFilterApplyingWayEh; override;
    ...
    function GetFilterListValuesLimit(Sender: TObject; DataSet: TDataSet): Integer; override;
    ...
    function FilterFieldNameToStrValue(FieldName: String; Column: TColumnEh; Grid: TCustomDBGridEh; DataSet: TDataSet; IsLocalFilter: Boolean): String; override;
  end;
...
var
  SortInView: Boolean;
  LocalFilterListValuesLimit: Integer;
...
function TMTEDatasetFeaturesEh.VarValueToFilterStrValue(
...
begin
  Result := inherited VarValueToFilterStrValue(AnOperator, Value, Column, Grid,
    DataSet, IsLocalFilter);
end;

function TMTEDatasetFeaturesEh.DateTimeValueToFilterStrValue(
...
begin
  ...
  begin
    Result := inherited DateTimeValueToFilterStrValue(AnOperator, Value, Column,
      Grid, DataSet, IsLocalFilter);
  end;
  {$ELSE}
  Result := inherited DateTimeValueToFilterStrValue(AnOperator, Value, Column,
    Grid, DataSet, IsLocalFilter);
...
function TMTEDatasetFeaturesEh.FilterFieldNameToStrValue(FieldName: String;
  Column: TColumnEh; Grid: TCustomDBGridEh; DataSet: TDataSet;
  IsLocalFilter: Boolean): String;
begin
  {$IFDEF FPC}
  if (IsLocalFilter = True) and (Column.Field.DataType in [ftDate,  ftDateTime]) then
  begin
    Result := 'DTOS('+FieldName+')';
  end else
  begin
    Result := inherited FilterFieldNameToStrValue(FieldName, Column, Grid, DataSet, IsLocalFilter);
  end;
  {$ELSE}
  Result := inherited FilterFieldNameToStrValue(FieldName, Column, Grid, DataSet, IsLocalFilter);
  {$ENDIF} 
...
function TMTEDatasetFeaturesEh.GetDataSetLikeWildcardForSeveralCharacters: String;
begin
  Result := '%';
end;

function TMTEDatasetFeaturesEh.GetLocalFilterApplyingWay: TLocalFilterApplyingWayEh;
begin
  {$IFDEF FPC}
...
function TMTEDatasetFeaturesEh.GetFilterListValuesLimit(Sender: TObject;
  DataSet: TDataSet): Integer;
begin
  if TCustomDBGridEh(Sender).STFilter.Local then
  begin
    Result := LocalFilterListValuesLimit;
  end
  else
  begin
    Result := -1;
...
initialization
  ...
  LocalFilterListValuesLimit := 1000;
end.
