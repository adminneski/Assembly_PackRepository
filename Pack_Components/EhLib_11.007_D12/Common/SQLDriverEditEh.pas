...
begin
  if DBServiceEngineList = nil then
    DBServiceEngineList := TObjectListEh.Create(True);
...
begin
  if DesignDataBaseList = nil then
    DesignDataBaseList := TObjectListEh.Create;
...
function TDesignDataBaseEh.BuildObjectTree(List: TObjectList): Boolean;
begin
  Result := False;
end;
...
begin
  ...
  Result.Name := 'mtDBTree';
  Result.FieldDefs.Add('Id', ftAutoInc);
  Result.FieldDefs.Add('RefParent', ftInteger);
  Result.FieldDefs.Add('Name', ftString, 500);
  Result.FieldDefs.Add('ChieldCount', ftInteger);
  Result.FieldDefs.Add('Description', ftString, 500);
  Result.FieldDefs.Add('RefData', ftUnknown);
  Result.FieldDefs.Add('ImageIndex', ftInteger);

  Result.TreeList.KeyFieldName := 'Id';
  Result.TreeList.RefParentFieldName := 'RefParent';
  Result.TreeList.DefaultNodeExpanded := False;
  Result.TreeList.DefaultNodeHasChildren := True;
  Result.TreeList.Active := True;
  Result.OnTreeNodeExpanding := mtDBTreeExpanding;
...
procedure TDesignDataBaseEh.mtDBTreeExpanding(Sender: TObject;
  ...
  begin
    ...
    begin
      sn := Node as TSQLTreeNode;
      if not sn.FNodesLoaded and (sn.FNodesTemplate <> nil) then
      begin
        List := sn.FNodesTemplate.FNodeManager.CreateNodes(Node as TSQLTreeNode);
        FTreeDataSet.TreeList.DefaultNodeHasChildren := sn.FNodesTemplate.FHasNodes;
        for i := 0 to List.Count-1 do
        begin
          FTreeDataSet.Append;
          FTreeDataSet.Fields[0].Value := Null;
...
begin
  ...
  begin
    ...
    if (Designer <> nil ) then
      Designer.Modified
    else if (DataDriver.Owner is TForm) and (TForm(DataDriver.Owner).Designer <> nil) then
...
begin
  ...
  YPos := (cp.Y+1) * FCharHeight;
  XPos := FSQLCanvas.TextWidth(Copy(Memo1.Lines[cp.Y], 1, cp.X)) - 3 ;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        vDBTree.TreeList.DefaultNodeHasChildren := sn.FNodesTemplate.FHasNodes;
        for i := 0 to List.Count-1 do
        begin
          vDBTree.Append;
...
function TCustomDBService.CreateRootNodes: TObjectList;
var
...
begin
  ...
  begin
    Node := CreateNode;
    stn := Node.GetSQLTreeNode;
    stn.FObjId := VarToStr(MemTable[Template.FObjIdFieldName]);
    stn.FName := VarToStr(MemTable[Template.FInTreeTextFieldName]);
    stn.FImageIndex := MemTable['IMAGE_INDEX'];
    stn.FMemRec := MemTable.RecordsView[MemTable.RecNo-1];
    stn.FNodesTemplate := GetTemplate(MemTable['TEMPLATE_NAME']);
    if stn.FNodesTemplate <> nil then
    begin
      stn.FTableFilter := stn.FNodesTemplate.FNodesFilter;
    end;
    stn.FNodesLoaded := False;
...
{$IFDEF DESIGNTIME}
  rif := TRegIniFile.Create(ActiveDesigner.Environment.GetBaseRegKey + '\' + EhLibRegKey);
  try
    SqlTextPathFileName := rif.ReadString('', 'sql_text', SqlTextPathFileName);
  finally
    rif.Free;
  end;
{$ENDIF}
  ...
  begin
    ...
    pd := TOpenDialog.Create(nil);
    if pd.Execute then
    begin
      SqlTextPathFileName := pd.FileName;
      Result := True;
{$IFDEF DESIGNTIME}
      rif := TRegIniFile.Create(ActiveDesigner.Environment.GetBaseRegKey + '\' + EhLibRegKey);
      try
        rif.WriteString('', 'sql_text', SqlTextPathFileName);
      finally
        rif.Free;
      end;
{$ENDIF}
...
function TCustomDBService.CreateNodes(Parent: TSQLTreeNode): TObjectList;
var
  MemTable: TMemTableEh;
  Reader: TDataSet;
  Node: TSQLTreeNode;
...
begin
  ...
  begin
    ...
    stn.FObjId := VarToStr(MemTable[ANodesTemplate.FObjIdFieldName]);
    stn.FName := VarToStr(MemTable[ANodesTemplate.FInTreeTextFieldName]);
    ...
    if stn.FNodesTemplate <> nil then
    begin
      stn.FTableFilter := VarToStr(MemTable[ANodesTemplate.FObjIdFieldName]);
      stn.FTableFilter := SubstitueFileds(stn.FMemRec, stn.FNodesTemplate.FNodesFilter);
    end;
    if ANodesTemplate <> nil then
    begin
      stn.OnDragDrop := ANodesTemplate.OnNodeDragDrop;
      stn.PopupParams := ANodesTemplate.NodeDropMenuParams;
    end;
    stn.FNodesLoaded := False;
    if UpperCase(MemTable['TEMPLATE_NAME']) = UpperCase('_RecordAsList') then
    begin
      stn.FParentRecordToTableMode := True;
      stn.FTable := GetMemTable('_RecordAsList');
      stn.FNodesLoaded := True;
    end;
...
begin
  inherited Create;
  FNodes := TObjectListEh.Create(True);
end;
...
begin
  ...
  t := TSQLTreeNodeTemplate.Create(Self, 'Informix Server');
  t.NodesSQLClassName :=    'ServerItems';
  t.MasterTemplateName :=   '';
  t.NodesMemTableName :=    'ServerItems';
  t.ObjIdFieldName :=       'OBJ_NAME';
  t.InTreeTextFieldName :=  'OBJ_NAME';
  t.HasNodes :=             True;
  t.NodesFilter :=          'aowner <> ''informix''';

  t := TSQLTreeNodeTemplate.Create(Self, 'Tables');
  t.NodesSQLClassName :=    'TableObjects';
  t.MasterTemplateName :=   'Tables';
  t.NodesMemTableName :=    'TableObjects';
  t.ObjIdFieldName :=       'tabid';
  t.InTreeTextFieldName :=  'name';
  t.HasNodes :=             True;
  t.NodesFilter :=          'atype = ''T'' and owner <> ''informix''';
  t.ColumnAttributesStr :=  '"TypeName", "Type", "100", '+ '"Description",  "Description", "200"';
  t.OnNodeDragDrop := t.TableEditorDrop;

  t := TSQLTreeNodeTemplate.Create(Self, 'TableColumns');
  t.NodesSQLClassName :=    'Columns';
  t.MasterTemplateName :=   '';
  t.NodesMemTableName :=    'TableColumns';
  t.ObjIdFieldName :=       'colno';
  t.InTreeTextFieldName :=  'colname';
  t.HasNodes :=             False;
  t.NodesFilter :=          '[tabid] = ''%[tabid]''';
  t.ColumnAttributesStr :=  '"colname", "Column name", "100", '+
    '"",  "Domain", "100", '+
    '"",  "Description", "100"';
  t.OnNodeDragDrop := TableEditorDrop;

  t := TSQLTreeNodeTemplate.Create(Self,'Views');
  t.NodesSQLClassName :=    'TableObjects';
  t.MasterTemplateName :=   'Views';
  t.NodesMemTableName :=    'TableObjects';
  t.ObjIdFieldName :=       'tabid';
  t.InTreeTextFieldName :=  'name';
  t.HasNodes :=             True;
  t.NodesFilter :=          'atype IN (''V'') and owner <> ''informix''';
  t.ColumnAttributesStr :=  '"Name", "Table name", "100", '+
    '"Description",  "Description", "200"';
  t.OnNodeDragDrop := t.TableEditorDrop;

  t := TSQLTreeNodeTemplate.Create(Self, 'ViewColumns');
  t.NodesSQLClassName :=    'Columns';
  t.MasterTemplateName :=   '';
  t.NodesMemTableName :=    'TableColumns';
  t.ObjIdFieldName :=       'colno';
  ...
  t := TSQLTreeNodeTemplate.Create(Self, 'Sinonims');
  t.NodesSQLClassName :=    'TableObjects';
  t.MasterTemplateName :=   'Sinonims';
  t.NodesMemTableName :=    'TableObjects';
  t.ObjIdFieldName :=       'tabid';
  t.InTreeTextFieldName :=  'name';
  t.HasNodes :=             False;
  t.NodesFilter :=          'atype IN (''P'', ''S'') and owner <> ''informix''';
  t.ColumnAttributesStr :=  '"Name", "Table name", "100", '+
                            '"Description",  "Description", "200"';
  t.OnNodeDragDrop := TableEditorDrop;

  t := TSQLTreeNodeTemplate.Create(Self, 'Procedures');
  t.NodesSQLClassName :=    'Procedures';
  t.MasterTemplateName :=   'Procedures';
  t.NodesMemTableName :=    'Procedures';
  t.ObjIdFieldName :=       'objname';
  t.InTreeTextFieldName :=  'name';
  t.HasNodes :=             False;
  t.NodesFilter :=          '';
  t.ColumnAttributesStr :=  '"TypeName", "Type", "100" ';
  t.OnNodeDragDrop := TableEditorDrop;

  t := TSQLTreeNodeTemplate.Create(Self, 'SysInfo');
  t.NodesSQLClassName :=    'SysInfo';
  t.MasterTemplateName :=   'SysInfo';
  t.NodesMemTableName :=    'ServerItems';
  t.ObjIdFieldName :=       'OBJ_NAME';
  t.InTreeTextFieldName :=  'OBJ_NAME';
  t.HasNodes :=             True;
  ...
  t := TSQLTreeNodeTemplate.Create(Self, 'SysTables');
  t.NodesSQLClassName :=    'SysTables';
  t.MasterTemplateName :=   'Tables';
  t.NodesMemTableName :=    'TableObjects';
  t.ObjIdFieldName :=       'tabid';
  t.InTreeTextFieldName :=  'name';
  ...
  t := TSQLTreeNodeTemplate.Create(Self, 'SysViews');
  t.NodesSQLClassName :=    'SysTables';
  t.MasterTemplateName :=   'Views';
  t.NodesMemTableName :=    'TableObjects';
  t.ObjIdFieldName :=       'tabid';
  ...
  t := TSQLTreeNodeTemplate.Create(Self, 'SysSinonims');
  t.NodesSQLClassName :=    'SysTables';
  t.MasterTemplateName :=   'Sinonims';
  t.NodesMemTableName :=    'TableObjects';
  t.ObjIdFieldName :=       'tabid';
  ...
  t := TSQLTreeNodeTemplate.Create(Self, 'SysProcedures');
  t.NodesSQLClassName :=    'SysProcedures';
  t.MasterTemplateName :=   'SysProcedures';
  t.NodesMemTableName :=    'SysProcedures';
  t.ObjIdFieldName :=       'objname';
  t.InTreeTextFieldName :=  'name';
  t.HasNodes :=             False;
  t.NodesFilter :=          '';
...
begin
  Result := TInformixSpecOperationsEh;
end;
...
begin
  ...
  t := TSQLTreeNodeTemplate.Create(Self, 'Interbase Server');
  t.NodesSQLClassName :=    'ServerItems';
  t.MasterTemplateName :=   '';
  t.NodesMemTableName :=    'ServerItems';
  t.ObjIdFieldName :=       'OBJ_NAME';
  ...
  t := TSQLTreeNodeTemplate.Create(Self, 'Tables');
  t.NodesSQLClassName :=    'Tables.Views';
  t.MasterTemplateName :=   'Tables';
  t.NodesMemTableName :=    'Table';
  t.ObjIdFieldName :=       'tabid';
  t.InTreeTextFieldName :=  'name';
  t.HasNodes :=             True;
  t.NodesFilter :=          '[RDB$VIEW_SOURCE] IS NULL';
  t.ColumnAttributesStr :=  '"Name", "Table name", "100", '+
  '"Description",  "Description", "200"';
  t.OnNodeDragDrop := t.TableEditorDrop;
  ...
  t.InTreeTextFieldName :=  'name';
  t.HasNodes :=             True;
  t.NodesFilter :=          '[RDB$VIEW_SOURCE] IS NOT NULL';
  t.OnNodeDragDrop := t.TableEditorDrop;

  t := TSQLTreeNodeTemplate.Create(Self, 'TableColumns');
  t.NodesSQLClassName :=    'TableColumns';
  t.MasterTemplateName :=   '';
  t.NodesMemTableName :=    'TableColumns';
  t.ObjIdFieldName :=       'colno';
  t.InTreeTextFieldName :=  'colname';
  t.HasNodes :=             False;
  t.NodesFilter :=          '[tabid] = ''%[name]''';
  t.ColumnAttributesStr :=  '';
  t.OnNodeDragDrop := t.TableEditorDrop;

  t := TSQLTreeNodeTemplate.Create(Self, 'TableColumnsProps');
  t.NodesSQLClassName :=    '';
  t.MasterTemplateName :=   'TableColumns';
  t.NodesMemTableName :=    '';
  t.ObjIdFieldName :=       '';
  t.InTreeTextFieldName :=  '';
  t.HasNodes :=             False;
  t.NodesFilter :=          '';
  t.ColumnAttributesStr :=  '';
  ...
  t := TSQLTreeNodeTemplate.Create(Self, 'Procedures');
  t.NodesSQLClassName :=    'Procedures';
  t.MasterTemplateName :=   'ServerItems';
  t.NodesMemTableName :=    'Procedures';
  t.ObjIdFieldName :=       'RDB$PROCEDURE_NAME';
  t.InTreeTextFieldName :=  'RDB$PROCEDURE_NAME';
  t.HasNodes :=             True;
  t.NodesFilter :=          '';
  t.OnNodeDragDrop := t.TableEditorDrop;

  t := TSQLTreeNodeTemplate.Create(Self, 'ProcedureParameters');
  t.NodesSQLClassName :=    'ProcedureParameters';
  t.MasterTemplateName :=   'Procedures';
  t.NodesMemTableName :=    'ProcedureParameters';
  t.ObjIdFieldName :=       'RDB$PARAMETER_NAME';
  t.InTreeTextFieldName :=  'RDB$PARAMETER_NAME';
  t.HasNodes :=             False;
  t.NodesFilter :=          '[RDB$PROCEDURE_NAME] = ''%[RDB$PROCEDURE_NAME]''';

  t := TSQLTreeNodeTemplate.Create(Self, 'Functions');
  t.NodesSQLClassName :=    'Functions';
  t.MasterTemplateName :=   'ServerItems';
  t.NodesMemTableName :=    'Functions';
  t.ObjIdFieldName :=       'RDB$FUNCTION_NAME';
  t.InTreeTextFieldName :=  'RDB$FUNCTION_NAME';
  t.HasNodes :=             True;
  t.NodesFilter :=          '';
  t.OnNodeDragDrop := t.TableEditorDrop;

  t := TSQLTreeNodeTemplate.Create(Self, 'FunctionParameters');
  t.NodesSQLClassName :=    'FunctionParameters';
  t.MasterTemplateName :=   'Functions';
  t.NodesMemTableName :=    'FunctionParameters';
  t.ObjIdFieldName :=       'RDB$ARGUMENT_POSITION';
  t.InTreeTextFieldName :=  'RDB$ARGUMENT_POSITION';
  t.HasNodes :=             False;
  t.NodesFilter :=          '[RDB$FUNCTION_NAME] = ''%[RDB$FUNCTION_NAME]''';

  t := TSQLTreeNodeTemplate.Create(Self, 'Generators');
  t.NodesSQLClassName :=    'Generators';
  t.MasterTemplateName :=   'ServerItems';
  t.NodesMemTableName :=    'Generators';
  t.ObjIdFieldName :=       'RDB$GENERATOR_NAME';
  t.InTreeTextFieldName :=  'RDB$GENERATOR_NAME';
  t.HasNodes :=             False;
  t.NodesFilter :=          '';
  t.OnNodeDragDrop := t.TableEditorDrop;
end;
...
begin
  ...
  t := TSQLTreeNodeTemplate.Create(Self, 'MSAccess');
  t.NodesSQLClassName :=    'ServerItems';
  t.MasterTemplateName :=   '';
  t.NodesMemTableName :=    'ServerItems';
  t.ObjIdFieldName :=       'OBJ_NAME';
  ...
  t.NodesMemTableName :=    'TableObjects';
  t.ObjIdFieldName :=       'tabid';
  t.InTreeTextFieldName :=  'objname';
  t.HasNodes :=             False;
  t.NodesFilter :=          '';
  t.ColumnAttributesStr :='"objname", "Name", "100", '+
    '"Description",  "Description", "200"';
...
begin
  ...
  t := TSQLTreeNodeTemplate.Create(Self, 'Oracle Server');
  t.NodesSQLClassName :=  'ServerItems';
  t.MasterTemplateName := '';
  t.NodesMemTableName :=  'ServerItems';
  t.ObjIdFieldName :=     'OBJ_NAME';
  ...
  t := TSQLTreeNodeTemplate.Create(Self, 'Tables');
  t.NodesSQLClassName :=  'Tables';
  t.MasterTemplateName := 'Tables';
  t.NodesMemTableName :=  'TableObjects';
  t.ObjIdFieldName :=     'OWNER_TABLE_NAME';
  t.InTreeTextFieldName :='VIS_TABLE_NAME';
  t.HasNodes :=           True;
  t.NodesFilter :=        '';
  t.ColumnAttributesStr :='"VIS_TABLE_NAME", "Table Name", "100" ' +
  ...
  t.NodesMemTableName :=  'TableColumns';
  t.ObjIdFieldName :=     'ID';
  t.InTreeTextFieldName :='COLUMN_NAME';
  t.HasNodes :=           False;
  t.NodesFilter :=        '[OWNER_TABLE_NAME] = ''%[OWNER_TABLE_NAME]''';
  t.ColumnAttributesStr :='"COLUMN_NAME", "Column name", "100", '+
    '"ID",  "ID", "50", '+
    '"",  "Description", "100"';

  t := TSQLTreeNodeTemplate.Create(Self, 'Views');
  t.NodesSQLClassName :=  'Views';
  t.MasterTemplateName := 'Views';
  t.NodesMemTableName :=  'ViewsObjects';
  t.ObjIdFieldName :=     'ID';
  t.InTreeTextFieldName :='VIS_OBJECT_NAME';
  t.HasNodes :=           True;
  t.NodesFilter :=        '';
  t.ColumnAttributesStr :='"VIS_OBJECT_NAME", "View name", "200", ';

  t := TSQLTreeNodeTemplate.Create(Self, 'Sequences');
  t.NodesSQLClassName :=  'Sequences';
  t.MasterTemplateName := 'Sequences';
  t.NodesMemTableName :=  'SequencesObjects';
  t.ObjIdFieldName :=     'ID';
  t.InTreeTextFieldName :='VIS_SEQUENCE_NAME';
  t.HasNodes :=           False;
  t.NodesFilter :=        '';
  t.ColumnAttributesStr :='"VIS_SEQUENCE_NAME", "Sequence name", "200" ';
...
begin
  ...
  t := TSQLTreeNodeTemplate.Create(Self, 'MSSQL Server');
  t.NodesSQLClassName :=  'ServerItems';
  t.MasterTemplateName := '';
  t.NodesMemTableName :=  'ServerItems';
  t.ObjIdFieldName :=     'OBJ_NAME';
  ...
  t.MasterTemplateName := 'Tables';
  t.NodesMemTableName :=  'TableObjects';
  t.ObjIdFieldName :=     'TABLE_NAME';
  t.InTreeTextFieldName :='TABLE_NAME';
  t.HasNodes :=           True;
  t.NodesFilter :=        '';
  t.ColumnAttributesStr :='"TABLE_NAME", "Table Name", "100" ';
  ...
  t.InTreeTextFieldName :='COLUMN_NAME';
  t.HasNodes :=           False;
  t.NodesFilter :=        '[TABLE_NAME] = ''%[TABLE_NAME]''';
  t.ColumnAttributesStr :='"COLUMN_NAME", "Column name", "100", '+
    '"DATA_TYPE",  "Type", "50", '+
  ...
  t.HasNodes :=           True;
  t.NodesFilter :=        '';
  t.ColumnAttributesStr :='"TABLE_NAME", "View name", "200" ';

  t := TSQLTreeNodeTemplate.Create(Self, 'Procedures');
  t.NodesSQLClassName :=  'Procedures';
  t.MasterTemplateName := 'Procedures';
  t.NodesMemTableName :=  'ProceduresObjects';
  t.ObjIdFieldName :=     'ROUTINE_NAME';
  t.InTreeTextFieldName :='ROUTINE_NAME';
  t.HasNodes :=           True;
  t.NodesFilter :=        '';
  t.ColumnAttributesStr :='"ROUTINE_NAME", "Routine name", "200" ';

  t := TSQLTreeNodeTemplate.Create(Self, 'Functions');
  t.NodesSQLClassName :=  'Functions';
  t.MasterTemplateName := 'Functions';
  t.NodesMemTableName :=  'FunctionsObjects';
  t.ObjIdFieldName :=     'ROUTINE_NAME';
  t.InTreeTextFieldName :='ROUTINE_NAME';
  t.HasNodes :=           True;
  t.NodesFilter :=        '';
  ...
  t := TSQLTreeNodeTemplate.Create(Self, 'Parameters');
  t.NodesSQLClassName :=  'Parameters';
  t.MasterTemplateName := '';
  t.NodesMemTableName :=  'Parameters';
  t.ObjIdFieldName :=     'ID';
  t.InTreeTextFieldName :='PARAMETER_NAME';
  t.HasNodes :=           False;
  t.NodesFilter :=        '[SPECIFIC_NAME] = ''%[ROUTINE_NAME]''';
  t.ColumnAttributesStr :='"PARAMETER_NAME", "Param name", "100", '+
    '"DATA_TYPE",  "Type", "50", '+
  ...
  begin
    ...
    try
      ...
      begin
        ...
        try
          if (Node.FNodesTemplate <> nil) and (Length(Node.FNodesTemplate.FColumnAttributes) > 0) then
          begin
            for i := 0 to Length(Node.FNodesTemplate.FColumnAttributes)-1  do
            begin
              col := gridTreeDetail.Columns.Add;
              col.FieldName := Node.FNodesTemplate.FColumnAttributes[i].FieldName;
              col.Title.Caption := Node.FNodesTemplate.FColumnAttributes[i].Title;
              col.Width := Node.FNodesTemplate.FColumnAttributes[i].Width;
              if i = 0 then
              begin
                col.ImageList := ImageList2;
                col.ShowImageAndText := True;
                if dsTreeDetail.DataSet.FindField('Image_Index') <> nil then
                  col.OnGetCellParams := gridTreeDetailColumns0GetCellParams;
              end;
            end;
          end else
            gridTreeDetail.Columns.RebuildColumns;
          if (dsTreeDetail.DataSet.FindField('Image_Index') <> nil) and
             (gridTreeDetail.Columns.Count > 0)
          then
          begin
            gridTreeDetail.Columns[0].ImageList := ImageList2;
            gridTreeDetail.Columns[0].ShowImageAndText := True;
            gridTreeDetail.Columns[0].OnGetCellParams := gridTreeDetailColumns0GetCellParams;
          end;
        finally
