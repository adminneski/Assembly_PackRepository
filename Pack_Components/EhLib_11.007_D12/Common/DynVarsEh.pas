...
{                 Dynamic Variables                     }
{                                                       }
{     Copyright (c) 2012-2022 by EhLib Team and         }
{                Dmitry V. Bolshakov                    }
{                                                       }
{*******************************************************}
...
begin
  ...
  if (ADinVar <> nil) and (ADinVar <> Self) then
    raise Exception.CreateResFmt(@EhLibLanguageConsts.DuplicateVarNameEh, [Value]);
  if IsDummy then
    FOwnerWhenDummy.CreateDynVarFromDummy(Self);
  FName := Value;
  Changed(False);
...
function TCustomDynVarEh.GetDisplayName: string;
var
  sName, sValue: String;
begin
  if (Name = '') and VarIsEmpty(Value) then
...
function TCustomDynVarsEh.GetInitDummyVar(const AVarName: String): TCustomDynVarEh;
begin
...
begin
  if Assigned(FindDynVar(Value.Name)) then
    raise Exception.CreateFmt(EhLibLanguageConsts.DuplicateVarNameEh, [Value.Name])
  else
...
begin
  ...
  if Assigned(Result) then
    raise Exception.CreateFmt(EhLibLanguageConsts.DuplicateVarNameEh, [VarName])
  else
    begin
...
var
  ADynVar: TCustomDynVarEh;
begin
  ADynVar := FindDynVar(VarName);
  if ADynVar <> nil then
    DeleteDynVar(ADynVar);
end;
...
procedure TStringVarEh.SetValue(const AValue: String);
begin
