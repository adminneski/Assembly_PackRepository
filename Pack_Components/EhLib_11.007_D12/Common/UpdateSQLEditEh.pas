...
function EditDataDriverUpdateSQL(ADataDriver: TCustomSQLDataDriverEh): Boolean;
var
  f: TUpdateSQLEditFormEh;
begin
  f := TUpdateSQLEditFormEh.Create(Application);
  try
    f.DataDriver := ADataDriver;
    Result := f.Edit;
  finally
    f.Free;
  end;
...
var
  sp: TSQLParser;
begin
  sp := TSQLParser.Create(SQL);
  try
    sp.GetSelectTableNames(List);
  finally
    sp.Free;
  end;
end;
...
begin
  sp := TSQLParser.Create(SQL);
  try
    sp.GetUpdateTableName(TableName);
    if Assigned(UpdateFields) then
    begin
      sp.Reset;
      sp.GetUpdateFields(UpdateFields);
    end;
    if Assigned(WhereFields) then
    begin
      sp.Reset;
      sp.GetWhereFields(WhereFields);
    end;
  finally
    sp.Free;
  end;
...
procedure TUpdateSQLEditFormEh.GenerateSQL;
  ...
  begin
    if ((UpdateTableName.ItemIndex <> -1) and
       ( UpdateTableName.Items.Objects[UpdateTableName.ItemIndex] <> nil)) or
         ((Pos('.', BaseName) > 0))
    then
...
var
  ...
  ls: TListBox;
begin
  ls := FieldListPopup.PopupComponent as TListBox;
  begin
