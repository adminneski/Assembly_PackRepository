...
begin
  {$IFDEF FPC}
  dpf := TDefinePropertyFiler.Create;
  {$ELSE}
...
procedure TPropStorageManagerEh.WritePropertiesText(PropStorage: TPropStorageEh; const Text: String);
begin
end;
...
begin
  ...
  try
    ...
    for i := 0 to sl.Count - 1 do
      begin
        n := Pos('=', sl[i]) + 2;
        sl[i] := Copy(sl[i], n, Length(sl[i]) - n);
      end;
...
begin
  ...
  begin
    FKey := Value;
    if FKey = HKEY_CLASSES_ROOT then
      FRegistryKey := rkClassesRootEh
    else if FKey = HKEY_CURRENT_USER then
      FRegistryKey := rkCurrentUserEh
    else if FKey = HKEY_LOCAL_MACHINE then
      FRegistryKey := rkLocalMachineEh
    else if FKey = HKEY_USERS then
      FRegistryKey := rkUsersEh
    else if FKey = HKEY_PERFORMANCE_DATA then
      FRegistryKey := rkPerformanceDataEh
    else if FKey = HKEY_CURRENT_CONFIG then
      FRegistryKey := rkCurrentConfigEh
    else if FKey = HKEY_DYN_DATA then
      FRegistryKey := rkDynDataEh
    else
...
procedure TRegPropStorageManEh.WritePropertiesText(PropStorage: TPropStorageEh; const Text: String);
var
  reg: TRegistry;
  sl: TStrings;
  i: Integer;
...
begin
  ...
  begin
    f := TForm(Form);
    FSaveFormShow := f.OnShow;
    f.OnShow := FormShow;
    FSaveFormCloseQuery := f.OnCloseQuery;
    f.OnCloseQuery := FormCloseQuery;
    FSaveFormDestroy := f.OnDestroy;
    f.OnDestroy := FormDestroy;
  end;
end;
...
begin
  ...
  begin
    f := TForm(Form);
    f.OnShow := FSaveFormShow;
    f.OnCloseQuery := FSaveFormCloseQuery;
    f.OnDestroy := FSaveFormDestroy;
  end;
end;
...
begin
  ...
  if CanClose and
     Active and
     (Owner is TCustomForm) and
     Form.HandleAllocated and
     FPropsLoaded
  then
    try
      SaveProperties;
...
begin
  if Active and not FSaved and FPropsLoaded then
  begin
    FDestroying := True;
    try
      SaveProperties;
...
procedure TPropStorageEh.ReadProp(Reader: TPropReaderEh; const PropName: String;
...
function TPropertyNamesEh.CheckPropertyPath(const Path: String): Boolean;
var
  Token: String;
...
begin
  ...
  CurObject := Root;
  Result := False;
  Token := GetNextPointSeparatedToken(APath);
  while True do
  begin
    if Token = '' then
      raise Exception.Create('Invalide property path: "' + APath + '"');
    if UpperCase(Token) = '<P>' then
    begin
      Result := CheckObjectPropertyPath(CurObject, Copy(APath, Length('<P>') + 2, Length(APath)));
      Exit;
    end;
...
{$IFDEF CIL}
    Borland.Delphi.System.Delete(APath, 1, Length(Token) + 1);
{$ELSE}
    System.Delete(APath, 1, Length(Token) + 1);
{$ENDIF}
    Token := GetNextPointSeparatedToken(APath);
  end;
end;

function TPropertyNamesEh.CheckObjectPropertyPath(Instance: TObject; const APropPath: String): Boolean;
var
  PropInfo: PPropInfo;
  PropName: String;
  dpl: TStringList;
  c: TCollection;
  i: Integer;
  ciList: TObjectListEh;
  InterceptorClass: TReadPropertyInterceptorClass;
  PropPath: String;

  function IsSpecCollectionPropName(const PropName: String): Boolean;
  begin
...
{$IFDEF FPC}
function TPropertyNamesEh.DoCompareText(const s1,s2 : string) : PtrInt;
{$ELSE}
...
{$IFDEF FPC}
  if DoCompareText(Strings[i], S) = 0 then
    Exit
  else if DoCompareText(Strings[i], S) > 0 then
{$ELSE}
