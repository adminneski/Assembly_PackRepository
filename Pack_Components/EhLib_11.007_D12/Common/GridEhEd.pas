...
{$ELSE}
  {$IFDEF FPC}
  GridAxisBarsDesignEditFormEh_Laz,
  {$ELSE}
  ComponentDesigner,
  LookupLinkDesignEh,
  EhLibDesignEditFavouriteComponents,
  Registry, Db, DBLookupUtilsEh, EhLibReg,
  {$ENDIF}
{$ENDIF}

{$ELSE}
{$ENDIF}
  EhLibDesignAbout, Dialogs;
...
{$IFDEF FPC}
var
  DBGrid: TCustomDBAxisGridEh;
begin
  case Index of
    0:
      begin
        DBGrid := GetComponent as TCustomDBAxisGridEh;
        EditDBGridColumns(DBGrid, DBGrid.AxisBars, 'AxisBars');
      end;
    1:
      ShowAboutForm;
  end;
  ...
  case Index of
    1:
      begin
        ...
        begin
          LMenuItem := AItem.AddItem(lst[i], 0, False, True, HandleCreateDataSourceSubMenu);
      ...
      begin
        ...
        begin
          ...
          if FDataSetList.Objects[i] = TCustomDBGridEh(Component).DataSource then
            S := S + ' (assigned)';
          LMenuItem := AItem.AddItem(S, 0, False, True, HandleAssignDataSourceSubMenu);
...
begin
{$IFDEF CIL}
  DBGridEh := TDBGridColumnsEh(Collection).Grid;
{$ELSE}
  DBGridEh := TDBGridColumnsEh(Collection).Grid;
{$ENDIF}
...
begin
  inherited Create;
  FGridList := TObjectListEh.Create;
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          MenuItem := TMenuItem.Create(DSPanel);
          MenuItem.Caption := 'EditButtons Collection Editor...';
          MenuItem.Tag := Longint(SelObject);
          MenuItem.OnClick := ShowEditButtonsCollectionWindow;
          DSPanel.DropdownMenu.Items.Add(MenuItem);

          MenuItem := TMenuItem.Create(DSPanel);
          MenuItem.Caption := 'CellButtons Collection Editor...';
          MenuItem.Tag := Longint(SelObject);
          MenuItem.OnClick := ShowCellButtonsCollectionWindow;
          DSPanel.DropdownMenu.Items.Add(MenuItem);

          DBGridEh.MouseCapture := False;
...
procedure TDBGridEhDesignControler.ShowEditButtonsCollectionWindow(Sender: TObject);
var
  AxisBar: TAxisBarEh;
  LDesigner: IDesigner;
begin
  ...
  if AxisBar = nil then Exit;
  if not Supports(FindRootDesigner(AxisBar.Grid), IDesigner, LDesigner) then Exit;
  ShowCollectionEditorClass(LDesigner, TCollectionEditor, AxisBar.Grid,
    TAxisBarEhCrack(AxisBar).EditButtons, 'EditButtons', [coAdd, coDelete, coMove]);
end;

procedure TDBGridEhDesignControler.ShowCellButtonsCollectionWindow(Sender: TObject);
var
  AxisBar: TAxisBarEh;
  LDesigner: IDesigner;
begin
  ...
  if not Supports(FindRootDesigner(AxisBar.Grid), IDesigner, LDesigner) then Exit;
  ShowCollectionEditorClass(LDesigner, TCollectionEditor, AxisBar.Grid,
    TAxisBarEhCrack(AxisBar).CellButtons, 'CellButtons', [coAdd, coDelete, coMove]);
end;
