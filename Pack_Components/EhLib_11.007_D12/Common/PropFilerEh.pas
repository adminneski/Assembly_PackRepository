...
begin
  for i := 0 to FDefnPropList.Count-1 do
    FreeObjectEh(FDefnPropList.Objects[i]);
  FreeAndNil(FDefnPropList);
  for i := 0 to FDefnBinPropList.Count-1 do
    FreeObjectEh(FDefnBinPropList.Objects[i]);
      ...
      begin
        ...
        begin
          ...
          for j := 0 to subsl.Count - 1 do
            sl.Add(PropInfo_getName(PropList[i]) + '.' + subsl[j]);
        end;
...
procedure TPropWriterEh.SaveObjectProperyValue(Instance: TObject; const APath, AFullPath: String);
var
  PropInfo: PPropInfo;
  PropType: PTypeInfo;
  PropName: String;
...
begin
  ...
  for i := FInterceptorList.Count-1 downto 0 do
  begin
    TStoragePropertyInterceptor(FInterceptorList[i]).Readed;
    FreeObjectEh(FInterceptorList[i]);
  end;
