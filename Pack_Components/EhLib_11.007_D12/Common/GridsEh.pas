...
{ TInplaceEditCoreControl }
  ...
  private
    ...
    procedure LMClear(var Message: TLMessage); message LM_CLEAR;
    {$ELSE}
...
{$ENDIF}
    ...
    {$IFDEF FPC}
    function CanCharShowEditor(Ch: TUTF8Char): Boolean; virtual;
    {$ELSE}
    ...
    function IsDrawCellSelectionThemed(ACol, ARow, DataCol, DataRow: Integer; State: TGridDrawState): Boolean; virtual;
    ...
    {$IFDEF FPC}
    procedure ShowEditorChar(Ch: TUTF8Char);
    {$ELSE}
...
procedure TInplaceEdit.DBCSKeyPress(var Key: String);
begin
...
{$IFDEF FPC}
procedure TInplaceEditCoreControl.LMPaste(var Message: TLMessage);
begin
  if not EditCanModify then Exit;
  if ParentEdit.DoPaste(Message) then
  begin
    Message.Result := 1;
  end else
  begin
...
procedure TInplaceEditCoreControl.LMCut(var Message: TLMessage);
begin
  if not EditCanModify then Exit;
...
procedure TInplaceEditCoreControl.LMClear(var Message: TLMessage);
begin
  if not EditCanModify then Exit;
...
begin
  ...
  begin
    {$IFDEF FPC}
    //The code in the Utf8KeyPress method will work.
    {$ELSE}
    DoDBCSKeyPress(Key);
    {$ENDIF}
    inherited KeyPress(Key);
...
{$IFDEF FPC}
procedure TInplaceEditCoreControl.Utf8KeyPress(var UTF8Key: TUTF8Char);
var
  StrKey: String;
begin
  StrKey := UTF8Key;
...
{$ELSE}
procedure TInplaceEditCoreControl.DoDBCSKeyPress(var Key: Char);
var
  CharMsg: TMsg;
  DBC: Boolean;
begin
...
procedure TInplaceEditCoreControl.DBCSKeyPress(var Key: String);
begin
  ParentEdit.DBCSKeyPress(Key);
end;
...
{$IFDEF FPC}
procedure TCustomGridEh.ShowEditorChar(Ch: TUTF8Char);
var
  Ch1: Char;
begin
  ShowEditor;
  ...
  begin
    if FInplaceEdit.IsMasked then
    begin
      if (Length(Ch) > 1) then
        FInplaceEdit.EditCoreControl.UTF8KeyPress(Ch)
      else
        FInplaceEdit.EditCoreControl.KeyPress(Ch[1]);
    end else
    begin
      FInplaceEdit.EditCoreControl.UTF8KeyPress(Ch);
      ...
      begin
        ...
        FInplaceEdit.SelStart := UTF8Length(Ch);
      end;
    end;
  end;
end;
...
{$IFDEF FPC}
function TCustomGridEh.CanCharShowEditor(Ch: TUTF8Char): Boolean;
{$ELSE}
...
function TCustomGridEh.IsDrawCellSelectionThemed(ACol, ARow, DataCol,
...
begin
  ...
  if UseRightToLeftAlignment then
    ChangeGridOrientation(Canvas, True, ClientWidth, ClientHeight);
  ...
  if UseRightToLeftAlignment then ChangeGridOrientation(Canvas, False, ClientWidth, ClientHeight);
...
procedure TCustomGridEh.CalcSizingState(X, Y: Integer;
  ...
  begin
    ...
    if (Axis.ContraStart = Axis.RolInClientBoundary) and
       (Pos >= Axis.RolInClientBoundary - Back) and
       (Pos <= Axis.RolInClientBoundary) then
    begin
      State := NewState;
...
procedure TCustomGridEh.ChangeGridOrientation(ACanvas: TCanvas; RightToLeftOrientation: Boolean);
var
  ClientSize: TSize;
begin
  {$IFDEF FPC}
  GetViewportExtEx(ACanvas.Handle, @ClientSize);
  {$ELSE}
  GetViewportExtEx(ACanvas.Handle, ClientSize);
  {$ENDIF}
  ChangeGridOrientation(ACanvas, RightToLeftOrientation, ClientSize.cx, ClientSize.cy);
end;

procedure TCustomGridEh.ChangeGridOrientation(ACanvas: TCanvas; RightToLeftOrientation: Boolean; AClientWidth: Integer; AClientHeight: Integer);
var
  Org: TPoint;
  Ext: TPoint;
begin
  ...
  begin
    Org := Point(AClientWidth, 0);
    Ext := Point(-1,1);
    SetMapMode(ACanvas.Handle, mm_Anisotropic);
    SetWindowOrgEx(ACanvas.Handle, Org.X, Org.Y, nil);
    SetViewportExtEx(ACanvas.Handle, AClientWidth, AClientHeight, nil);
    SetWindowExtEx(ACanvas.Handle, Ext.X*AClientWidth, Ext.Y*AClientHeight, nil);
...
{$ENDIF}
begin
  FInternalColor := CheckSysColor(Color);
  FInternalFontColor := CheckSysColor(Font.Color);
  FInternalFixedColor := CheckSysColor(FixedColor);
  FInternalFixedFontColor := CheckSysColor(Font.Color);
{$IFDEF EH_LIB_16}
...
begin
  ...
  begin
    EditorText := FInplaceEdit.Text;
    SetEditText(FInplaceCol, FInplaceRow, EditorText);
  end;
end;
...
{$IFDEF FPC}
function TCustomGridEh.DoUTF8KeyPress(var UTF8Key: TUTF8Char): boolean;
begin
  Result := inherited DoUTF8KeyPress(UTF8Key);
  if (goEditingEh in Options) and
     ((UTF8Key = ^H) or (UTF8Key >= #32)) and
     CanCharShowEditor(UTF8Key) and
     CanEditModify then
  begin
    ShowEditorChar(UTF8Key);
    UTF8Key := '';
    Result := true
  end;
end;
...
begin
  if (FromIndex <= FixedCelCount-1) and
     (ToIndex <= FixedCelCount-1)
  then
    ArrayMove(FFixedCelLens, FromIndex, ToIndex)
  else if (FromIndex >= FixedCelCount) and
          (FromIndex <= CelCount-1) and
          (ToIndex >= FixedCelCount) and
          (ToIndex <= CelCount-1) then
  begin
    ArrayMove(FRolCelLens, FromIndex-FixedCelCount, ToIndex-FixedCelCount);
    FRolLocCelPosArrObsolete := True;
  end
  else if (FromIndex >= CelCount) and
          (FromIndex <= FullCelCount-1) and
          (ToIndex >= CelCount) and
          (ToIndex <= FullCelCount-1)
  then
...
begin
  ...
  begin
    Result := Size;
    Result := FGrid.ScaleValue(Result);
  end else
  begin
    Result := FGrid.ScrollBarSize;
  end;
end;
...
begin
  ...
  begin
    if Result > FGrid.GetSystemMetrics(SM_CYHSCROLL) then
      Result := FGrid.GetSystemMetrics(SM_CYHSCROLL);
  end;
end;
...
begin
  ...
  FVisibleIterms := TStringList.Create;
end;
...
procedure TCustomListboxEh.DefaultDrawItem(VisibleIndex: Integer; ARect: TRect;
  State: TGridDrawState);
var
  ImRect: TRect;
  ImageIndex: Integer;
...
begin
  ...
  begin
    if (VisibleItems.Count > 0)
      then s := GetDisplayText(VisibleIndex)
      else s := '';

    Canvas.Font := Font;

    if (SelectionDrawParams.GetActualSelectionStyle = gsdsClassicEh) and
       ((VisibleIndex = ItemVisibleIndex) or (gdSelected in State)) then
    begin
      Canvas.Font.Color := StyleServices.GetSystemColor(clHighlightText);
      Canvas.Brush.Color := StyleServices.GetSystemColor(clHighlight)
    end else
    ...
    if SelectionDrawParams.GetActualSelectionStyle <> gsdsClassicEh then
      DrawCellDataBackground(0, VisibleIndex, ARect, State);
    WriteTextEh(Canvas, ARect, False, 2, 0, s, ItemAlignment, tlTop, False, False, 0, 0, UseRightToLeftAlignment, True);

    if (FFilterStr <> '') then
    begin
      DrawHighlightedSubTextEh(Canvas, ARect, 2, 0, s, ItemAlignment, tlTop,
        False, False, 0, 0, UseRightToLeftAlignment, FFilterStr,
        FFilterCaseInsensitive, False, FFilterType = lsftBeginsWithEh,
        clYellow, 0, clYellow, ofv);
    end;
  end else
  begin
    ...
    ImageIndex := VisibleIndexToDataIndex(VisibleIndex);
    if Assigned(OnGetImageIndex) then
      OnGetImageIndex(Owner, ImageIndex, ImageIndex);
    ...
    if UseRightToLeftAlignment
      then ItemAlignment := taRightJustify
      else ItemAlignment := taLeftJustify;
    ...
    s := GetDisplayText(VisibleIndex);
    if VisibleIndex < VisibleItems.Count then
      WriteTextEh(Canvas, ARect, False, 2, 0, s, ItemAlignment, tlTop, False, False, 0, 0, UseRightToLeftAlignment, True);
...
function TCustomListboxEh.GetDisplayText(VisibleIndex: Integer): String;
var
  DataIndex: Integer;
begin
  DataIndex := VisibleIndexToDataIndex(VisibleIndex);
  if (DataIndex < Items.Count) then
  begin
    if UseItemObjects and
       (Items.Objects[DataIndex] is TCustomListboxItemEh)
    then
      Result := TCustomListboxItemEh(Items.Objects[DataIndex]).GetDisplayText(Self, DataIndex)
    else
      GetData(DataIndex, Result);
  end;
end;
...
var
  AItemVisibleIndex: Integer;
begin
  AItemVisibleIndex := ItemVisibleIndex;
  if (AItemVisibleIndex >= 0) then
    Result := VisibleIndexToDataIndex(AItemVisibleIndex)
  else
    Result := -1;
end;

function TCustomListboxEh.GetItemVisibleIndex: Integer;
begin
  if FRowSeleced
    then Result := Row
    else Result := -1;
...
begin
  if (FFilterStr = '') then
    ItemVisibleIndex := Value
  else
    ItemVisibleIndex := DataIndexToVisibleIndex(Value);
end;

procedure TCustomListboxEh.SetItemVisibleIndex(const Value: Integer);
begin
...
begin
  Result := VisibleIndexToDataIndex(TopVisibleIndex);
end;

function TCustomListboxEh.GetTopVisibleIndex: Integer;
begin
  Result := TopRow;
end;
...
var
  AItemVisibleIndex: Integer;
begin
  AItemVisibleIndex := VisibleItemAtPos(Pos, Existing);
  Result := VisibleIndexToDataIndex(AItemVisibleIndex);
end;

function TCustomListboxEh.VisibleItemAtPos(Pos: TPoint; Existing: Boolean): Integer;
begin
...
function TCustomListboxEh.VisibleItemRect(VisibleIndex: Integer): TRect;
begin
  Result := CellRect(0, VisibleIndex);
end;
...
begin
  if (VisibleItems.Count = 0)
    then RowCount := 1
    else RowCount := VisibleItems.Count;
...
begin
  ...
  begin
    AItemRect := VisibleItemRect(AItemIndex);
...
function TCustomListboxEh.StrictVisibleIndexToClient(X, Y: Integer): Integer;
begin
  if (X < 0) or (X >= ClientWidth) then
    X := 0;
  Result := VisibleItemAtPos(Point(X, Y), True);
  if Y < 0 then
    Result := TopIndex;
  if Y >= ClientHeight then
  begin
...
function TCustomListboxEh.GetVisibleIterms: TStrings;
begin
  Result := FVisibleIterms;
end;

procedure TCustomListboxEh.ApplyFilter(AFilterStr: String;
  AFilterType: TLSAutoFilterTypeEh; ACaseInsensitive: Boolean);
var
  OldDataIndex: Integer;
begin
  OldDataIndex := ItemIndex;
  FFilterStr := AFilterStr;
  FFilterType := AFilterType;
  FFilterCaseInsensitive := ACaseInsensitive;
  ResetVisibleIterms;
  UpdateRowCount;
  AdjustMaxTopLeft(True, True, not IsSmoothHorzScroll, not IsSmoothVertScroll);
  if (DataIndexToVisibleIndex(OldDataIndex) >= 0)
    then ItemIndex := OldDataIndex
    else ItemIndex := -1;
  Invalidate;
end;

procedure TCustomListboxEh.ClearFilter();
var
  OldDataIndex: Integer;
begin
  if FFilterStr = '' then Exit;

  OldDataIndex := ItemIndex;
  FFilterStr := '';
  FFilterType := lsftBeginsWithEh;
  FFilterCaseInsensitive := False;
  ResetVisibleIterms;
  UpdateRowCount;
  ItemIndex := OldDataIndex;
  Invalidate;
end;

procedure TCustomListboxEh.ResetVisibleIterms;
var
  BaseList: TStrings;
  i: Integer;
  IntAsObj: TObject;
begin
  BaseList := GetItems;
  FVisibleIterms.BeginUpdate;
  try
    FVisibleIterms.Clear;
    for i := 0 to BaseList.Count - 1 do
    begin
      if ValueIsHitFilter(BaseList[i]) then
      begin
        IntAsObj := IntPtrToObject(i);
        FVisibleIterms.AddObject(BaseList[i], IntAsObj);
      end;
    end;
  finally
    FVisibleIterms.EndUpdate;
  end;
end;

function TCustomListboxEh.ValueIsHitFilter(Value: String): Boolean;
var
  FoundPos: Integer;
  SubStr: String;
  ValueStr: String;
begin
  ...
  if (FFilterCaseInsensitive) then
  begin
    SubStr := NlsUpperCase(FFilterStr);
    ValueStr := NlsUpperCase(Value);
  end else
  begin
    SubStr := FFilterStr;
    ValueStr := Value;
  ...
  if (FFilterCaseInsensitive)
    then FoundPos := RoughStringPosProcEh(SubStr, ValueStr, 1)
    else FoundPos := Pos(SubStr, ValueStr);

  if (FFilterType = lsftBeginsWithEh) then
  begin
    Result := (FoundPos = 1);
  end else
  begin
...
function TCustomListboxEh.VisibleIndexToDataIndex(VisibleIndex: Integer): Integer;
var
  DataIdxObject: TObject;
begin
  ...
  begin
    DataIdxObject := VisibleItems.Objects[VisibleIndex];
    Result := ObjectToIntPtr(DataIdxObject);
  end;
end;

function TCustomListboxEh.DataIndexToVisibleIndex(DataIndex: Integer): Integer;
var
  i: Integer;
  DataIdxObject: TObject;
  DataIdx: Integer;
begin
  ...
  begin
    ...
    begin
      DataIdxObject := VisibleItems.Objects[i];
      DataIdx := ObjectToIntPtr(DataIdxObject);
      if (DataIdx = DataIndex) then
      begin
        Result := i;
        Exit;
      end;
...
begin
  inherited;
  if ItemVisibleIndex >= 0 then
  begin
    AItemRect := VisibleItemRect(ItemVisibleIndex);
    WindowsInvalidateRect(Handle, AItemRect, False);
  end;
end;
...
begin
  ...
  begin
    ...
    if Rows < 1 then
      Rows := 1;
    FRowCount := Rows;
    AdjustedHeight := Rows * TextHeight + BorderSize;
  end
  else
  begin
    AdjustedHeight := AHeight;
  end;
  inherited SetBounds(ALeft, ATop, AWidth, AdjustedHeight);
end;
...
function TPopupListboxFormEh.GetSizeGripResized: Boolean;
begin
  Result := ListGrid.SizeGripResized;
end;

procedure TPopupListboxFormEh.SetSizeGripResized(const Value: Boolean);
begin
  ListGrid.SizeGripResized := Value;
end;
...
procedure TPopupListboxFormEh.WMSize(var Message: TWMSize);
begin
  inherited;
  if not InternalResizing then
  begin
...
procedure TPopupListboxFormEh.ApplyFilter(AFilterStr: String;
  AFilterType: TLSAutoFilterTypeEh; ACaseInsensitive: Boolean);
begin
  ListGrid.ApplyFilter(AFilterStr, AFilterType, ACaseInsensitive);
end;

procedure TPopupListboxFormEh.ClearFilter();
begin
  ListGrid.ClearFilter;
end;
...
begin
  inherited MouseMove(Shift, X, Y);
  VisibleIndex := VisibleItemAtPos(Point(X, Y), True);
  if VisibleIndex = -1 then
    VisibleIndex := StrictVisibleIndexToClient(X, Y);
  if (FMousePos.X <> X) or (FMousePos.Y <> Y) then
  begin
    if VisibleIndex >= 0 then
      ItemVisibleIndex := VisibleIndex;
    if ([ssLeft, ssRight, ssMiddle] * Shift = [])  and
       (FMouseVisibleIndex <> VisibleIndex) then
    begin
      if FMouseVisibleIndex < VisibleItems.Count then
        InvalidateIndex(FMouseVisibleIndex);
      FMouseVisibleIndex := VisibleIndex;
      InvalidateIndex(FMouseVisibleIndex);
    end;
  end;
  FMousePos := Point(X, Y);
