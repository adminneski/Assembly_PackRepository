...
{                                                       }
{   Copyright (c) 2014-2022 by Dmitry V. Bolshakov      }
{                                                       }
{*******************************************************}
...
uses EhLibLangConsts, DBAxisGridsEh, EditPivotFieldFormEh;
...
begin
  ...
  TMTStringDataFieldEh(bf).StringDataType := fdtWideStringEh;
...
begin
  ...
  begin
    EditPivotField(TPivotFieldValueInfoEh(TRefObjectField(FDataTable.Fields[1]).Value), FToolBox.VisibleAggregateFunctions);
  end;
end;
      ...
      begin
        ...
        begin
          ...
          if GetMoveLineEh.Visible
            ...
            else GetMoveLineEh.StartShow(ScreenPos, False, LineSize, Self);
        end;
      end else if ListType = sgftRowFieldsEh then
      begin
        Accept := True;
...
begin
  ...
  begin
    ...
    Params.Text := Params.Text + ' (' + PivotAggrValueDisplayNames(spi.SumFunction) + ')';
  end;
end;
...
{$IFDEF EH_LIB_16}
  ShowCaption := False;
{$ENDIF}
  FVisibleAggregateFunctions := [svtSumEh, svtCountEh, svtAvgEh,
  ...
  FFieldListPanel.ControlStyle := FFieldListPanel.ControlStyle - [csAcceptsControls];
  ...
  FPivotDataPanel.ControlStyle := FPivotDataPanel.ControlStyle - [csAcceptsControls];
  ...
  FRowsLabel.Caption := EhLibLanguageConsts.PivotRowsEh;
  ...
  FColsLabel.Caption := EhLibLanguageConsts.PivotColumnsEh;
  ...
  FValsLabel.SetBounds(10, FColsListGrid.Top+FColsListGrid.Height+10,
                       FPivotDataPanel.Width-20, 30);
  FValsLabel.Caption := EhLibLanguageConsts.PivotValuesEh;
...
procedure TCustomPivotGridToolBoxEh.PivotDataStartChanging(Sender: TObject);
begin
end;

function TCustomPivotGridToolBoxEh.PivotDataChangeProgress(Sender: TObject;
  ElapsedTime: LongWord; Percent: Integer): Boolean;
begin
  Result := False;
end;

procedure TCustomPivotGridToolBoxEh.PivotDataFinishChanging(Sender: TObject);
begin
end;

procedure TCustomPivotGridToolBoxEh.PivotDataChangingCanceled(Sender: TObject);
begin
end;
