...
{                                                       }
{    Copyright (c) 2003-2022 by Dmitry V. Bolshakov     }
{                                                       }
{*******************************************************}
...
uses SysUtils, Classes, Controls, DB, Forms, Contnrs,
  Variants,
{$IFDEF FPC}
  EhLibLCL, LCLType,
  {$IFDEF FPC_CROSSP}
...
{ TDataDriverEh }
  ...
  TDataDriverEh = class(TComponent, ISideOwnedComponentEh, IDataDriverConsumerEh, IUnknown)
  private
  ...
  public
    ...
    procedure AssignFieldValue(MemTableData: TMemTableDataEh; MemRec: TMemoryRecordEh; DataFieldIndex: Integer; DataValueVersion: TDataValueVersionEh; ReaderDataSet: TDataSet); virtual;
    ...
    procedure DefaultUpdateError(MemTableData: TMemTableDataEh; MemRec: TMemoryRecordEh; var Action: TUpdateErrorActionEh); virtual;
...
{ TBaseSQLDataDriverEh }
  ...
  protected
    ...
    procedure AssignCommandParam(Command: TBaseSQLCommandEh; MemRecord: TMemoryRecordEh; DataValueVersion: TDataValueVersionEh; Param: TParam); virtual;
  ...
  public
    ...
    procedure DefaultAssignCommandParam(Command: TBaseSQLCommandEh; MemRecord: TMemoryRecordEh; DataValueVersion: TDataValueVersionEh; Param: TParam); virtual;
...
{ TBaseSQLCommandEh }
  ...
  public
    ...
    procedure Assign(Source: TPersistent); override;
    procedure DefaultRefreshParam(MemRecord: TMemoryRecordEh; DataValueVersion: TDataValueVersionEh; Param: TParam); virtual;
...
{ IDesignDataBaseEh }
  ...
  ['{01F477A4-8417-4DC9-B93A-1F95D2FF2EB8}']
    ...
    function BuildObjectTree(List: TObjectList): Boolean;
...
{ TInformixSpecOperationsEh }

  TInformixSpecOperationsEh = class(TServerSpecOperationsEh)
  public
    class function GetServerTypeName: String; override;
  public
    procedure GetBackUpdatedValues(MemRec: TMemoryRecordEh; Command: TCustomSQLCommandEh; ResDataSet: TDataSet); override;
...
var
  ...
  DesignDataBuilderClasses: TClassList;
...
begin
  ...
  RegisterServerType(TInformixSpecOperationsEh);
...
begin
  ...
  if ServerTypeClasses.IndexOf(STName) >= 0 then
    raise Exception.Create('Server type w ith name: "'+STName+'" is already registered');

  ServerTypeClasses.AddObject(STName, __TObject(ServerTypeClass));
end;
        ...
        begin
          ...
          if Action = ueaBreakRaiseEh then
            ...
            if MemRec.UpdateError <> nil then
              FreeObjectEh(MemRec.UpdateError);
        ...
        try
          ...
          begin
            ...
            if Assigned(ProviderField) and not ProviderField.ReadOnly then
              ProviderField.Value := MemRec.Value[i, dvvValueEh];
          end;
...
begin
  ...
  begin
    Self.CommandText := (Source as TCustomSQLCommandEh).CommandText;
    Self.CommandType := (Source as TCustomSQLCommandEh).CommandType;
  end;
end;
...
begin
  ...
  begin
    Self.ParamCheck := (Source as TBaseSQLCommandEh).ParamCheck;
    Self.Params := (Source as TBaseSQLCommandEh).Params;
  end
  else if Source is TCustomSQLCommandEh then
  begin
    Self.Params.Assign((Source as TCustomSQLCommandEh).GetParams);
  end;
end;
...
begin
  ...
  begin
    DesignDataBuilderClasses := TClassList.Create;
    DesignDataBuilderProcs := TList.Create;
  end;
  ExistsIdx := DesignDataBuilderClasses.IndexOf(DataDriverClass);
  if ExistsIdx >= 0 then
    DesignDataBuilderProcs[ExistsIdx] := @DesignDataBaseProc
  else
  begin
    DesignDataBuilderClasses.Add(DataDriverClass);
...
procedure TCustomSQLDataDriverEh.ApplyOrderByText(const OrderByText: String);
begin
  if OrderByText = '' then
...
procedure TCustomSQLDataDriverEh.ApplyFilterText(const FilterText: String);
begin
  if FilterText = '' then
...
function TCustomSQLDataDriverEh.CheckAddFieldInDynaSQL(
  UpdateStatus: TUpdateStatus; const FieldName: String): Boolean;
begin
  Result := True;
...
begin
  ...
  try
    ...
    DynaSQLParams.GetUpdateFields(UpdateFieldList, MemRec);
...
procedure TServerSpecOperationsEh.GenWhereClause(const KeyFields: String; SQL: TStrings);
var
...
constructor TInformixSpecOperationsEh.Create;
begin
  inherited Create;
end;

procedure TInformixSpecOperationsEh.GetBackUpdatedValues(MemRec: TMemoryRecordEh;
  Command: TCustomSQLCommandEh; ResDataSet: TDataSet);
const
...
class function TInformixSpecOperationsEh.GetServerTypeName: String;
begin
  Result := 'Informix';
end;
...
begin
  inherited Create;
  FDataDriver := ADataDriver;
  FUpdateFieldListSource := tflsListPropertyEh;
end;
...
procedure TDynaSQLParamsEh.GetUpdateFields(List: TStrings; MemRec: TMemoryRecordEh);
var
  ConnProvider: IConnectionProviderEh;
  Schema: IDatabaseSchemaInfoEh;
  kl, sl: TStringList;
  i, ki: Integer;
begin
  if UpdateFieldListSource = tflsDataBaseSchemaEh then
  begin
    ConnProvider := DataDriver.GetConnectionProvider;
    if ConnProvider <> nil then
    begin
      sl := TStringList.Create;
      try
      Schema := ConnProvider.GetDatabaseSchemaInfo;
      if Schema <> nil then
        Schema.GetFieldNames(UpdateTable, sl);
      kl := TStringList.Create;
      try
        kl.Delimiter := ',';
        kl.CommaText := KeyFields;
      ...
      for i := 0 to MemRec.DataStruct.Count-1 do
      begin
        ki := sl.IndexOf(MemRec.DataStruct[i].FieldName);
        if ki >= 0 then
          List.Add(sl[ki]);
      end;
      finally
...
begin
  ...
  begin
    ...
    UpdateFieldListSource := TDynaSQLParamsEh(Source).UpdateFieldListSource;
  end else
    inherited Assign(Source);
end;
  ...
  begin
    ...
    begin
      ...
      if NewSTIndex < 0 then
        raise Exception.Create('Server type w ith name "'+Value+'" does not exist.');
    {$IFDEF FPC}
      FServerSpecOperationsClass := {%H-}TServerSpecOperationsEhClass(__Int(ServerTypeClasses.Objects[NewSTIndex]));
    {$ELSE}
      FServerSpecOperationsClass := TServerSpecOperationsEhClass(__Int(ServerTypeClasses.Objects[NewSTIndex]));
    {$ENDIF}
...
function TConnectionProviderEh.ExecuteSQL(const SQLText: String; AParams: TParams;
  var Processed: Boolean): Integer;
begin
  Processed := False;
  Result := 0;
...
procedure TConnectionProviderEh.FillMemTableForSQL(const SQLText: String;
...
function TConnectionProviderEh.GetDatabaseSchemaInfo: IDatabaseSchemaInfoEh;
begin
  Result := nil;
end;

procedure TConnectionProviderEh.GetDataReaderForSQL(const SQLText: String;
...
procedure TMacroVarsEh.ScanStrForMacros(const SrcStr: String; var sl: TStringList);
var
  i: Integer;
begin
...
function TMacroVarsEh.SubstituteMacro(const Text, MacroName, MacroValue: String): String;
var
