...
{ TDBLookupDataEh }
  ...
  TDBLookupDataEh = class(TPersistent, IMTEventReceiverEh)
  private
  ...
  protected
    ...
    procedure CheckListLinkIMemTable; virtual;
  ...
  public
    constructor Create(AColumn: TCollectionItem); reintroduce;
    destructor Destroy; override;
    ...
    procedure RefreshLookupListCache; virtual;
...
begin
  ...
  begin
    ...
    FLookupData.CheckListLinkIMemTable;
  end;
end;
...
procedure TDBLookupDataEh.CheckListLinkIMemTable;
var
...
begin
  ...
  if (ListLink <> nil) and
     ListLink.Active and
     (ListLink.DataSet <> nil) and
     not (csDestroying in ListLink.DataSet.ComponentState)
  then
    Supports(ListLink.DataSet, IMemTableEh, FIntMemTable)
  else
    FIntMemTable := nil;
...
var
  ...
  OldListDataset: TDataSet;
  OldLookupKeyFields: TFieldsArrEh;
  OldLookupDisplayField: TField;

  function CheckListDefinitionChanged(): Boolean;
  var
    i: Integer;
  begin
    Result := False;
    if OldListDataset <> ListDataset then
      Result := True
    else if OldLookupDisplayField <> FLookupDisplayField then
      Result := True
    else if Length(OldLookupKeyFields) <> Length(FLookupKeyFields) then
      Result := True
    else
    begin
      for i := 0 to Length(FLookupKeyFields)-1 do
        if (FLookupKeyFields[i] <> OldLookupKeyFields[i]) then
        begin
          Result := True;
          Break;
        end;
...
begin
  ...
  OldListDataset := FLookupLink.DataSet;
  OldLookupKeyFields := FLookupKeyFields;
  OldLookupDisplayField := FLookupDisplayField;
  ...
  if (ALookupListActive <> FLookupListActive) or
     (CheckListDefinitionChanged() and not FDataFieldsUpdating) then
  begin
...
function TDBLookupDataEh.GetKeyValue: Variant;
begin
  DataSetGetFieldValues(DataLink.DataSet, KeyFields, Result);
end;

function TDBLookupDataEh.GetMTLookupValue(RowNum: Integer): Variant;
var
  FIntMemTable: IMemTableEh;
begin
  if Supports(DataLink.DataSet, IMemTableEh, FIntMemTable) then
  begin
    FIntMemTable.InstantReadEnter(RowNum);
    try
...
function TDBLookupDataEh.GetKeyFieldsIsNull: Boolean;
var
  I: Integer;
begin
  ...
  begin
    ...
    for i := 0 to Length(KeyFields)-1 do
      if not KeyFields[i].IsNull then
      begin
        Result := False;
        Exit;
      end;
...
var
  WasActive: Boolean;
  Bookmar: TUniBookmarkEh;
begin
  FLookupListCache.Clear;

  if LookupListActive and
     LookupCache and
     (LookupDataSet.State = dsBrowse) then
  begin
    WasActive := LookupDataSet.Active;
    try
      LookupDataSet.DisableControls;
      try
        Bookmar := LookupDataSet.Bookmark;
        LookupDataSet.First;
        ...
        if DataSetBookmarkValid(LookupDataSet, Bookmar) then
          LookupDataSet.Bookmark := Bookmar;
      finally
...
procedure TDBLookupDataEh.MTViewDataEvent(RowNum: Integer;
...
{$IFDEF FPC}
function TDBLookupDataEh.QueryInterface(constref IID: TGUID;
  out Obj): HResult;
{$ELSE}
function TDBLookupDataEh.QueryInterface(const IID: TGUID;
  out Obj): HResult;
{$ENDIF}
begin
...
function TDBLookupDataEh._AddRef: Integer;
begin
  Result := -1;
end;

function TDBLookupDataEh._Release: Integer;
begin
  Result := -1;
end;
...
var
  LookupPair: TLookupPairEh;
begin
  LookupPair := TLookupPairEh.Create;
...
begin
  if FindKeyValueIndex(AKeyValue, Idx) then
    Result := TLookupPairEh(FList[Idx]).DisplayValue
  else
    Result := Unassigned;
end;
...
begin
  ...
  begin
    I := (L + H) shr 1;
    C := DBVarCompareValue(TLookupPairEh(FList[i]).KeyValue, KeyValue);
  ...
  repeat
    ...
    repeat
      while DBVarCompareValue(
              TLookupPairEh(FList[I]).KeyValue,
              TLookupPairEh(FList[P]).KeyValue) = vrLessThan do
        Inc(I);
      while DBVarCompareValue(
              TLookupPairEh(FList[J]).KeyValue,
              TLookupPairEh(FList[P]).KeyValue) = vrGreaterThan do
        Dec(J);
      if I <= J then
      begin
        FList.Exchange(I, J);
