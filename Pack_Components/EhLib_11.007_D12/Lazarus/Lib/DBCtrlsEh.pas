...
uses
 Messages, Contnrs, Variants, StrUtils,
  ...
  ToolCtrlsEh, ButtonsEh, ActnList, Math, DynVarsEh, DropDownFormEh;
...
{ TCustomDBEditEh }
  ...
  protected
    ...
    function DataFieldValueToText(AFieldValue: Variant): String; virtual;
    ...
    property OnButtonDown: TEditButtonDownEventEh read FOnButtonDown write FOnButtonDown;
...
{ TCustomDBNumberEdit }
  ...
  private
    ...
    function GetPopupCalculatorClass: TWinControlClass;
    ...
    procedure SetPopupCalculatorClass(const Value: TWinControlClass);
  ...
  public
    ...
    property PopupCalculatorClass: TWinControlClass read GetPopupCalculatorClass write SetPopupCalculatorClass;
...
{ TCustomDBMemoEh }
  ...
  protected
    ...
    function DataFieldValueToText(AFieldValue: Variant): String; override;
...
{ TCustomDBRichEditEh }
  ...
  protected
    ...
    procedure UpdateEditButtonControlList;
    procedure UpdateEditButtonControlsState;
    procedure UpdateBaseReadOnlyState; virtual;
...
begin
  ...
  if Color <> clDefault
    then GetControl.Canvas.Brush.Color := CheckSysColor(Color)
    else GetControl.Canvas.Brush.Color := CheckSysColor(GetControl.GetFillColor);
...
begin
  ...
  if (not DataLink.Editing) and CanModify then
  begin
    case Key of
      ^H, ^V, ^X, #32..High(Char):
        FDataLink.Edit;
...
function TCustomDBEditEh.CanModify: Boolean;
begin
...
var
  ...
  ClickTime: LongInt;
  KeyMsg: TWMKey;
  CurMessageResult: LRESULT;
begin

  if FInplaceMode then
  begin
    CurMessageResult := Message.Result;
    Message.Result := 0;
    ...
    Message.Result := CurMessageResult;
  end;
...
{$ENDIF}
        begin
          ...
          if (Message.Msg = WM_CHAR) and (KeyMsg.CharCode = VK_BACK) then
            FKeepMRUList := True;
...
function TCustomDBEditEh.DataFieldValueToText(AFieldValue: Variant): String;
begin
  Result := VarToStr(AFieldValue);
end;
...
begin
  ...
  begin
    ...
    ATextValue := DataFieldValueToText(AValue);

    if FFocused and FDataLink.CanModify then
    begin
      InternalSetText(ATextValue);
    end else
    begin
      if EditMask <> '' then
        EditText := FormatMaskTextEh(EditMask, ATextValue)
      else if (EditText <> ATextValue) then
        EditText := ATextValue;
    end;
  end
  ...
  begin
    if not (evEditMaskEh in FAssignedValues) then
      SetControlEditMask('');
    EditText := DataFieldValueToText(FDataLink.DataIndependentValue);
  end else
  begin
    if not (evEditMaskEh in FAssignedValues) then
      SetControlEditMask('');
...
begin
  ...
  if (OldText <> Text) and
     HandleAllocated and
     (GetWindowLong(Handle, GWL_STYLE) and ES_MULTILINE <> 0)
  then
...
procedure SynchronizeFocus(Control: TWinControl);
var
  Parent: TCustomForm;
begin
  ...
  if Parent <> nil then
  begin
    if Parent.Active and
       (Control = Parent.ActiveControl) and
       Control.HandleAllocated and
...
begin
  ...
  if (Message.Sender <> Self) and
     not ContainsControl(Message.Sender) and
     (Message.Sender <> FMRUListControl) and
     not CheckDataListChilds
  then
    MRUList.CloseUp(False);
end;
...
begin
  ...
  if not ControlRec.EditButton.Visible then
    ...
    ControlRec.EditButtonControl.Width := ControlRec.EditButtonControl.GetSystemMetrics(SM_CXVSCROLL);
...
begin
  ...
  if MRUList.DroppedDown and
     (Text = BlankText) and
     not FKeepMRUList then
  begin
    MRUList.CloseUp(False);
  end
          ...
          ((Text <> BlankText) or FKeepMRUList) and
          FFocused
  then
  begin
    MRUList.DropDown;
...
procedure TCustomDBEditEh.WSSetText(const AText: String);
var
  OldVal: TCaption;
begin
  OldVal := Text;
...
procedure TCustomDBEditEh.ChangeScale(M, D: Integer {$IFDEF EH_LIB_24}; isDpiChange: Boolean {$ENDIF});
{$IFDEF FPC}
{$ELSE}
var
  Flags: TScalingFlags;
...
begin
  ...
  begin
    ...
    {$ENDIF}
    try
      if not EmptyDataInfo.ParentFont then
        EmptyDataInfo.Font.Size := MulDiv(EmptyDataInfo.Font.Size, M, D);
    finally
    end;
  end;
...
function CorrectDate(var Year, Month, Day: Integer): Boolean; overload;
var
...
function CorrectDate(var Year, Month, Day: Word): Boolean; overload;
var
  YearInt, MonthInt, DayInt: Integer;
begin
  YearInt := Year; MonthInt := Month; DayInt := Day;
  Result := CorrectDate(YearInt, MonthInt, DayInt);
  Year := YearInt; Month := MonthInt; Day := DayInt;
end;

function CorrectTime(var Hor, Min, Sec: Integer): Boolean; overload;
begin
  Result := False;
...
function CorrectTime(var Hor, Min, Sec: Word): Boolean; overload;
var
  HorInt, MinInt, SecInt: Integer;
begin
  HorInt := Hor; MinInt := Min; SecInt := Sec;
  Result := CorrectTime(HorInt, MinInt, SecInt);
  Hor := HorInt; Min := MinInt; Sec := SecInt;
end;
...
begin
  DecodeDateTime(0, Y, M, D, H, N, S, MS);
  dts := DateTimeStamp;
...
procedure TCustomDBDateTimeEditEh.DropDownAction(EditButton: TEditButtonEh;
  EditButtonControl: TEditButtonControlEh; var Handled: Boolean);
begin
  if (FCalendarVisible = False) and (FIgnoreNextDropDown = True) then
  begin
...
begin
  ...
  begin
    ...
    PostMessage(Handle, CM_IGNOREEDITDOWN, WPARAM(FButtonsBox.BtnCtlList[0].EditButtonControl.Tag), 0);
...
begin
  ...
  begin
    ...
    begin
      ...
      MousePos.y := 0; 
      SendMessage(DropDownCalendar.Handle, WM_LBUTTONDOWN, 0, LPARAM(SmallPointToInteger(MousePos)));
    end;
  end;
end;
...
begin
  ...
  if (FItems is TCustomComboBoxStringsEh) then
    TCustomComboBoxStringsEh(FItems).ComboBox := Self;
...
begin
  ...
  try
    ...
    SetPopupListboxFilterFromSelection;
  except
...
procedure TCustomDBComboBoxEh.SetPopupListboxFilterFromSelection;
var
  FilterText: String;
begin
  if (ListVisible) and DropDownBox.AutoFilter then
  begin
    FilterText := Copy(Text, 1, SelStart);
    TComboBoxPopupListboxEh(PopupListbox).ApplyFilter(FilterText, DropDownBox.AutoFilterType, CaseInsensitiveTextSearch);
  end;
end;
...
begin
  ...
  begin
    DataLinkValAsStr := VarToStr(FDataLink.DataIndependentValue);
    if not (evEditMaskEh in FAssignedValues) then
      SetControlEditMask('');
    if FKeyBased then
      InternalSetValue(FDataLink.DataIndependentValue)
    else if (EditText <> DataLinkValAsStr) then
      EditText := DataLinkValAsStr;
  end else
  begin
    if not (evEditMaskEh in FAssignedValues) then
      SetControlEditMask('');
...
begin
  ...
  begin
    ...
    if (Images <> nil) and
       (EditImage.UseImageHeight) and
       (ppls.ItemHeight < Images.Height + 1)
    then
...
begin
  ...
  begin
    ...
    if ppls.SizeGripResized then
    begin
      DropDownBox.Rows := ppls.RowCount;
      DropDownBox.Width := ppls.Width;
    end;
    ...
    if Accept and not ReadOnly and FDataLink.Edit then
    begin
      InternalSetItemIndex(ppls.ItemIndex);
      if FFocused then SelectAll;
    end;
...
begin
  ...
  begin
    ...
    begin
      TControl(Sender).Perform(WM_CANCELMODE, 0, 0);
      MousePos := PointToSmallPoint(ListPos);
      SendMessage(PopupListbox.Handle, WM_LBUTTONDOWN, 0, LPARAM(SmallPointToInteger(MousePos)));
      Result := True;
    end;
  end;
end;
...
begin
  ...
  if (Message.Sender <> Self) and not
     ContainsControl(Message.Sender) and
     (Message.Sender <> PopupListbox) and
     not CheckDataListChilds
  then
    CloseUp(False);
end;
      ...
      begin
        ...
        if MouseMsg.Msg = WM_RBUTTONUP then
          Perform(WM_CONTEXTMENU, Handle,
            LPARAM(SmallPointToInteger(PointToSmallPoint(
...
var
  PopupListFocus: Boolean;
begin
  PopupListFocus := ((Message.FocusedWnd = PopupListbox.Handle) or
                 WinServiceIsChild(PopupListbox.Handle, Message.FocusedWnd)) and
                 PopupListbox.CanFocus();
  if FListVisible and not PopupListFocus then
    CloseUp(False);
  inherited;
end;
...
function TCustomDBComboBoxEh.GetItemsClass: TStringsClass;
begin
  Result := TCustomComboBoxStringsEh;
end;
...
begin
  ...
  begin
    FDropDownCalculator := PopupCalculatorClass.Create(Self);
  end;
...
procedure TCustomDBNumberEditEh.DropDownAction(EditButton: TEditButtonEh;
  EditButtonControl: TEditButtonControlEh; var Handled: Boolean);
begin
  if (FCalculatorVisible = False) and (FIgnoreNextDropDown = True) then
  begin
    FIgnoreNextDropDown := False;
    Exit;
  end;
...
function TCustomDBNumberEditEh.GetPopupCalculatorClass: TWinControlClass;
begin
  if (FPopupCalculatorClass <> nil) then
    Result := FPopupCalculatorClass
  else
    Result := EhLibManager.PopupCalculatorClass;
end;

procedure TCustomDBNumberEditEh.SetPopupCalculatorClass(const Value: TWinControlClass);
begin
  if (FPopupCalculatorClass <> Value) then
  begin
    if FDropDownCalculator <> nil then
      FreeAndNil(FDropDownCalculator);
    FPopupCalculatorClass := Value;
  end;
end;
...
{$IFDEF FPC}
  ...
  {$ELSE}
  Canvas.Brush.Color := CheckSysColor(Parent.Color);
...
begin
  ...
  begin
    ...
    if ButtonDownInControlState then
      Perform(WM_LBUTTONUP, 0, LPARAM($FFFFFFFF));
  end;
end;
...
begin
  ...
  begin
    ...
    Memo.SetEditMode;
    SendTextMessage(Memo.Handle, EM_REPLACESEL, 0, Empty);
//    {$HINTS OFF}
...
function TCustomDBMemoEh.DataFieldValueToText(AFieldValue: Variant): String;
begin
  Result := inherited DataFieldValueToText(AFieldValue);
...
{$ENDIF}
  ...
  then
  begin
    Canvas.Brush.Color := CheckSysColor(clWindowFrame);
...
begin
  ...
  MaxButtonHeight := Round(16 * ScaleFactor);
  EditButtonControlLineRec.EditButton := FEditButton;
  ResetEditButtonControl(EditButtonControlLineRec, 0, False, MaxButtonHeight, MinButtonHeight);
...
begin
  ...
  GetControl.Canvas.Brush.Color := CheckSysColor(GetPaintColor);
...
begin
  ...
  FButtonsBox.Parent := Self;
  FButtonsBox.Visible := False;
  FButtonsBox.ParentBackground := False;
  FButtonsBox.SetBounds(0,0,0,0);
  FButtonsBox.OnDown := EditButtonDown;
  FButtonsBox.OnClick := EditButtonClick;
  FButtonsBox.OnMouseMove := EditButtonMouseMove;
...
begin
  ...
  begin
    ...
    {$IFDEF EH_LIB_17} //Delphi XE3
    if (seFont in StyleElements) and CustomStyleActive then
    begin
...
procedure TCustomDBRichEditEh.UpdateBaseReadOnlyState;
begin
  // Due to a bug in the control, we leave the algorithm for working with the property in the basic state.
...
begin
  SetFocused(True);
  inherited;
  inherited ReadOnly := not FDataLink.CanModify or ReadOnly;
end;
...
procedure TCustomDBRichEditEh.SelectionChange;
begin
...
procedure TCustomDBRichEditEh.WMHScroll(var Message: TWMHScroll);
begin
  inherited;
  UpdateEditButtonControlList;
end;

procedure TCustomDBRichEditEh.WMVScroll(var Message: TWMVScroll);
begin
  inherited;
  UpdateEditButtonControlList;
end;
...
procedure TControlLabelEh.ChangeScale(M, D: Integer {$IFDEF EH_LIB_24}; isDpiChange: Boolean {$ENDIF});
begin
  inherited ChangeScale(M, D{$IFDEF EH_LIB_24}, isDpiChange{$ENDIF});
end;
...
{$IFDEF EH_LIB_16} // Delphi XE2
  TCustomStyleEngine.RegisterStyleHook(TCustomDBMemoEh, TMemoStyleHook);
{$ENDIF}
...
finalization
{$IFDEF EH_LIB_16} // Delphi XE2
  TCustomStyleEngine.UnRegisterStyleHook(TCustomDBMemoEh, TMemoStyleHook);
{$ENDIF}
end.
