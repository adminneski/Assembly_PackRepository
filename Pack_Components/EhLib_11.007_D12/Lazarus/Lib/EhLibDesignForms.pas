...
{                                                       }
{    Copyright (c) 1913-2022 by Dmitry V. Bolshakov     }
{                                                       }
{*******************************************************}
...
uses
  {$IFDEF FPC}
    ...
    NewItemIntf, ProjectIntf, FormEditingIntf,
    {$IFDEF FPC_CROSSP}
    {$ELSE}
      Windows, UxTheme, MultiMon,
    {$ENDIF}
  {$ELSE}
    EhLibVCL, Windows, UxTheme, MultiMon, DesignIntf, DesignEditors, ToolsAPI,
  {$ENDIF}
...
{ TEhLibFormFileDescriptor }

  TEhLibFormFileDescriptor = class(TFileDescPascalUnitWithResource)
  private
  public
  ...
  constructor TEhLibFormFileDescriptor.Create;
  begin
    inherited Create;
    ResourceClass := TCustomDropDownFormEh;
    FormEditingHook.RegisterDesignerBaseClass(TCustomDropDownFormEh);
    Name := 'TCustomDropDownFormEh';
    RequiredPackages := 'EhLib';
  end;

  function TEhLibFormFileDescriptor.GetLocalizedName: String;
  begin
    Result := 'TCustomDropDownFormEh';
  end;

  function TEhLibFormFileDescriptor.GetLocalizedDescription: String;
  begin
    Result := 'EhLib Dev Team';
  end;

  function TEhLibFormFileDescriptor.GetInterfaceUsesSection: String;
  begin
    Result := inherited GetInterfaceUsesSection;
    Result := Result + ', Forms, DropDownFormEh';
  end;
...
begin
{$IFDEF FPC}
  RegisterNewItemCategory(TNewIDEItemCategory.Create('EhLib'));
  RegisterProjectFileDescriptor(TEhLibFormFileDescriptor.Create, 'EhLib');
  FormEditingHook.RegisterDesignerBaseClass(TCustomDropDownFormEh);
{$ELSE}
