...
{$ELSE}
  ...
  {$ELSE}
  EhLibVCL, SqlTimSt, DBCommon, DBConsts, Windows,
  {$ENDIF}
{$ENDIF}
  Variants, FMTBcd,
...
uses
  DateUtils, RTLConsts, DBUtilsEh, MemTableEh;
...
function EncodeISO8601Time(AYear, AMonth, ADay, AHour, AMinute, ASecond, AMillisecond: Word;
  AHourOffset, AMinuteOffset: Integer): String;
const
  SDateTimeFormat: string = '%.4d-%.2d-%.2dT%.2d:%.2d:%.2d'; { Do not localize }
  SDateFullTimeFormat: string = '%.4d-%.2d-%.2dT%.2d:%.2d:%.2d.%.3d'; { Do not localize }
  SOffsetFormat: string = '%s%.2d:%.2d'; { Do not localize }
  Neg: array[Boolean] of string = ('+', '-'); { Do not localize }
var
  Bias: Integer;
begin
  if AMillisecond = 0 then
    Result := Format(SDateTimeFormat, [AYear, AMonth, ADay, AHour, AMinute, ASecond])
  else
    Result := Format(SDateFullTimeFormat, [AYear, AMonth, ADay, AHour, AMinute, ASecond, AMillisecond]);

  Bias := AHourOffset * 60 + AMinuteOffset;
  if (AHourOffset <> 0) or (AMinuteOffset <> 0) then
    Result := Result + Format(SOffsetFormat, [Neg[Bias < 0], AHourOffset, AMinuteOffset]);
end;

{$IFDEF FPC}
{$ELSE}
function VarSQLTimeStampToISO8601(const v: Variant): String;
var
  sts: TSQLTimeStamp;
begin
  sts := VarToSQLTimeStamp(v);
  Result := EncodeISO8601Time(sts.Year, sts.Month, sts.Day,
    sts.Hour , sts.Minute, sts.Second, sts.Fractions, 0, 0);
end;
{$ENDIF}

{$IFDEF EH_LIB_13}
function VarSQLTimeStampOffsetToISO8601(const v: Variant): String;
var
  sts: TSQLTimeStampOffset;
begin
  sts := VarToSQLTimeStampOffset(v);
  Result := EncodeISO8601Time(sts.Year, sts.Month, sts.Day,
    sts.Hour , sts.Minute, sts.Second, sts.Fractions,
    sts.TimeZoneHour, sts.TimeZoneMinute);
end;
{$ENDIF}
...
procedure DecodeISO8601DateTime(const AISODateTime: String;
  var AYear: {$IFDEF EH_LIB_13}Word{$ELSE}SmallInt{$ENDIF};
  var AMonth, ADay, AHour, AMinute, ASecond: Word;
  var AMillisecond: {$IFDEF EH_LIB_22}Cardinal{$ELSE}System.LongWord{$ENDIF};
  var AHourOffset, AMinuteOffset: SmallInt);
var
  s: String;
  p: Integer;
begin
  s := Copy(AISODateTime, 1, 4);
  AYear := StrToInt(s);

  s := Copy(AISODateTime, 6, 2);
  AMonth := StrToInt(s);

  s := Copy(AISODateTime, 9, 2);
  ADay := StrToInt(s);

  s := Copy(AISODateTime, 12, 2);
  if s <> ''
    then AHour := StrToInt(s)
    else AHour := 0;

  s := Copy(AISODateTime, 15, 2);
  if s <> ''
    then AMinute := StrToInt(s)
    else AMinute := 0;

  s := Copy(AISODateTime, 18, 2);
  if s <> ''
    then ASecond := StrToInt(s)
    else ASecond := 0;

  if Copy(AISODateTime, 20, 1) = '.' then
  begin
    s := Copy(AISODateTime, 21, 3);
    AMillisecond := StrToInt(s);
    p := 24;
  end else
  begin
    AMillisecond := 0;
  ...
  s := Copy(AISODateTime, p, 1);
  if (s = '+') or (s = '-') then
  begin
    s := Copy(AISODateTime, p+1, 2);
    AHourOffset := StrToInt(s);

    s := Copy(AISODateTime, p+4, 2);
    AMinuteOffset := StrToInt(s);
  end else
  begin
    AHourOffset := 0;
    AMinuteOffset := 0;
...
{$IFDEF FPC}
{$ELSE}
function ISO8601ToVarSQLTimeStamp(const s: String): Variant;
var
  AHourOffset, AMinuteOffset: SmallInt;
  sts: TSQLTimeStamp;
begin
  DecodeISO8601DateTime(s,
    sts.Year, sts.Month, sts.Day, sts.Hour, sts.Minute, sts.Second, sts.Fractions,
    AHourOffset, AMinuteOffset);
  Result := VarSQLTimeStampCreate(sts);
end;
{$ENDIF}

{$IFDEF EH_LIB_13}
function ISO8601ToVarSQLTimeStampOffset(const s: String): Variant;
var
  sts: TSQLTimeStampOffset;
begin
  DecodeISO8601DateTime(s,
    sts.Year, sts.Month, sts.Day, sts.Hour, sts.Minute, sts.Second, sts.Fractions,
    sts.TimeZoneHour, sts.TimeZoneMinute);
  Result := VarSQLTimeStampOffsetCreate(sts);
end;
{$ENDIF}
...
begin
  ...
  begin
    { TODO : Make Implementation for  NEXTGEN }
  end;
end;
{$ELSE}
{$IFDEF CIL}
...
procedure TRecordsViewOrderByList.AssignFieldIndex(OrderItem: TOrderByItemEh;
  const FieldIndex: Integer);
begin
  OrderItem.MTFieldIndex := FieldIndex;
end;
...
function TMemTableDataEh.CreateRecordsList: TRecordsListEh;
begin
  Result := TRecordsListEh.Create(Self);
end;
...
procedure TMemTableDataEh.UpdateCalculatedField(MemRec: TMemoryRecordEh);
var
  VarValues: TVariantArrayEh;
  i: Integer;
begin
  if Assigned(FOnFieldCalculation) then
  begin
    SetLength(VarValues, DataStruct.Count);
    OnFieldCalculation(MemRec, VarValues);
    for i := 0 to DataStruct.Count-1 do
    begin
      if DataStruct[i].Calculated then
        if not VarIsClear(VarValues[i]) then
          MemRec.Value[i, dvvValueEh] := VarValues[i]
        else
          MemRec.Value[i, dvvValueEh] := Null;
    end;
  end;
end;
...
var
  i: Integer;
begin
  if Action in [rlnRecAddedEh, rlnRecChangedEh] then
    UpdateCalculatedField(MemRec);
...
procedure TMemTableDataEh.SetOnFieldCalculation(
  const Value: TMemoryRecordEventEh);
var
  i: Integer;
begin
  if @FOnFieldCalculation <> @Value then
  begin
    FOnFieldCalculation := Value;
    for i := 0 to RecordsList.Count-1 do
      UpdateCalculatedField(RecordsList[i]);
  end;
end;
...
begin
  ...
  begin
    RemoveComponent(TComponent(FList[i]));
    FreeObjectEh(FList[i]);
  end;
  FList.Clear;
...
begin
  ...
  Result.Calculated := AFieldDef.InternalCalcField;
end;
...
procedure TMTDataStructEh.BuildFieldDefsFromStruct(FieldDefs: TFieldDefs);
  ...
  begin
    ...
    begin
      ...
      {$IFDEF FPC}
      if F is TMTStringDataFieldEh then
        CodePage := TMTStringDataFieldEh(F).CodePage
      else
        CodePage := 0;

      FieldDef := TFieldDef.Create(FieldDefs, F.FieldName, F.DataType, F.Size, F.Required, F.Index + 1, CodePage);
      FieldDef.InternalCalcField := F.Calculated;
...
var
  I: Integer;
  Fields: TObjectListEh;
begin
  Result := nil;
  SetLength(Result, 0);
  if Pos(';', FieldNames) <> 0 then
  begin
    Fields := TObjectListEh.Create;
    try
  ...
  begin
    ...
    begin
      if CharInSetEh(FieldName[I], ['A'..'Z','a'..'z','_','0'..'9']) then
        Inc(I)
      else if IsLeadCharEh(FieldName[I]) then
...
begin
  ...
  begin
    ...
    Calculated := TMTDataFieldEh(Source).Calculated;
  end else
    inherited Assign(Source);
end;
...
begin
  ...
  Calculated := (Field.FieldKind = fkInternalCalc);
end;
...
{$ENDIF}
    ...
    ftWideString: Result := WideStringVarType;
...
procedure TMTDataFieldEh.SetCalculated(const Value: Boolean);
begin
  if FCalculated <> Value then
  begin
    FCalculated := Value;
    PropertyChanged;
  end;
...
{$ENDIF}
  ...
  begin
    {$IFDEF FPC}
    if (Reader.NextValue = vaLString) then
      v := Reader.ReadString
    else
    ...
    begin
      NeedVarType := GetVarDataType;
      {$IFDEF FPC}
      {$ELSE}
      ValueVarType := VarType(v);
      {$ENDIF}
      if NeedVarType = VarFMTBcd then
        v := SysStrToVar(v, VarFMTBcd)
{$IFDEF FPC}
      ;
{$ELSE}
      else if (NeedVarType = varSQLTimeStamp) and
         (
{$IFDEF EH_LIB_12}
          (ValueVarType = varUString) or
{$ENDIF}
          (ValueVarType = varOleStr) or
          (ValueVarType = varString))
      then
        v := ISO8601ToVarSQLTimeStamp(VarToStr(v))
{$IFDEF EH_LIB_13}
      else if (NeedVarType = VarSQLTimeStampOffset) and
         (
{$IFDEF EH_LIB_12}
          (ValueVarType = varUString) or
{$ENDIF}
          ...
          (ValueVarType = varString))
      then
        v := ISO8601ToVarSQLTimeStampOffset(VarToStr(v))
{$ENDIF} 
      ;
{$ENDIF} 
      if NeedVarType = varVariant then
        Value := v
      else
...
{$ELSE}
      if VarType(Value) = varSQLTimeStamp then
        Writer.WriteString(VarSQLTimeStampToISO8601(Value))
      else
{$ENDIF}
{$IFDEF EH_LIB_13}
      if VarType(Value) = varSQLTimeStampOffset then
        Writer.WriteString(VarSQLTimeStampOffsetToISO8601(Value))
      else
{$ENDIF}
      if VarType(Value) = VarFMTBcd then
        Writer.WriteString(SysVarToStr(Value))
      else
...
begin
  ...
  begin
    ...
    {$IFDEF FPC}
    Codepage := TStringField(Field).Codepage;
    {$ELSE}
    {$ENDIF}
  end
end;
...
begin
  ...
  begin
    ...
    if FStringDataType = fdtGuidEh then
      FSize := 38;
    PropertyChanged;
    FVarDataType := GetVarDataType;
  end;
...
{$ENDIF}
{$IFDEF EH_LIB_13}
    ftSingle: FNumericDataType := fdtSingleEh;
{$ENDIF}
  else
    raise Exception.Create('Can not assign DataType from Field');
  end;
...
begin
  if NumericDataType in [fdtBCDEh, fdtFMTBcdEh]
    then Result := True
    else Result := False;
end;
...
begin
  {$IFDEF FPC}
  if FieldType in [Low(ftBlobTypes)..High(ftBlobTypes)] then
    FBlobType := FieldType;
  {$ELSE}
...
function TMTBlobDataFieldEh.GetBlobType: TBlobType;
begin
  {$IFDEF FPC}
  Result := TBlobType{%H-}(FBlobType);
  {$ELSE}
  Result := TBlobType(FBlobType);
  {$ENDIF}
end;
...
{$ENDIF}
{$IFDEF EH_LIB_13}
    ftTimeStampOffset: FSQLTimeStampDataFieldType := fdtTimeStampOffsetEh;
{$ENDIF}
  else
    raise Exception.Create('Can not assign DataType from Field');
  end;
...
procedure TMTSQLTimeStampDataFieldEh.SetSQLTimeStampDataFieldType(const
  Value: TSQLTimeStampDataFieldTypesEh);
begin
  if FSQLTimeStampDataFieldType <> Value then
  begin
    FSQLTimeStampDataFieldType := Value;
    FVarDataType := GetVarDataType;
  end;
end;
...
begin
  InernalMergeChanges(True);
end;

procedure TMemoryRecordEh.InernalMergeChanges(RemoveDeleted: Boolean);
begin
  if UpdateStatus = usUnmodified then Exit;
  if UpdateStatus = usDeleted then
    if RemoveDeleted
      then RecordsList.PersistRemoveRecord(Self.Index)
      else RecordsList.DeleteRecordNilPos(Self.Index);
  FOldData := nil;
  FUpdateStatus := usUnmodified;
...
procedure TMemoryRecordEh.UpdateCalculatedFields;
begin
  RecordsList.MemTableData.UpdateCalculatedField(Self);
end;
...
begin
  ...
  FDeltaList := TObjectListEh.Create;
  FNewHashCode := 1;
  FRecList := TObjectListEh.Create;
  FDeletedList := TObjectListEh.Create;
...
procedure TRecordsListEh.DeleteRecordNilPos(Index: Integer);
var
  DelRec: TMemoryRecordEh;
begin
  DelRec := Rec[Index];
...
begin
  ...
  begin
    ...
    case Rec.UpdateStatus of
      usModified:
        begin
          if Rec.FOldData = nil then Exit;
          Rec.FData := Rec.FOldData;
          Rec.FOldData := nil;
          Rec.FUpdateStatus := usUnmodified;
          Rec.UpdateIndex := -1;
        end;
      usInserted:
        begin
          DeltaIndex := FDeltaList.IndexOf(Rec);
          if DeltaIndex >= 0 then
            FDeltaList[DeltaIndex] := nil;
          Rec := Delete(Rec.Index);
          FreeObjectEh(Rec);
        end;
      usDeleted:
        begin
...
begin
  ...
  try
    if DeltaList.Count = 1 then
    begin
      Rec := TMemoryRecordEh(DeltaList[0]);
      if (Rec <> nil) then
        Rec.MergeChanges;
    end else
    begin
      ...
      begin
        Rec := TMemoryRecordEh(DeltaList[i]);
        if (Rec <> nil) then
          Rec.InernalMergeChanges(False);
      end;
    end;
    FreeDeletedRecords;
    PackRecList;
...
procedure TRecordsListEh.PackRecList;
var
  OldCount: Integer;
begin
  OldCount := FRecList.Count;
  FRecList.Pack;
  if OldCount <> FRecList.Count then
    ReIndexRecs(0, Count-1);
end;
...
procedure TRecordsListEh.RecListExchange(Index1, Index2: Integer);
begin
  FRecList.Exchange(Index1, Index2);
end;
...
begin
  if not EhLibDebugChecks then Exit;
...
begin
  ...
  FMemoryViewList := TObjectListEh.Create;
  FSorderViewList := TObjectListEh.Create;
...
begin
  ...
  FreeAndNil(FSorderViewList);
  while FNotificators.Count > 0 do
    TRecordsListNotificatorEh(FNotificators[0]).DataObject := nil;
  FreeAndNil(FNotificators);
  FreeAndNil(FAggregates);
  FreeAndNil(FMemoryTreeList);
  FOrderByList.ClearFreeItems;
...
var
  ...
  IdxInSorderList: Integer;
  PostNiMemRec: TMemoryRecordEh;
  PostNiIndex: Integer;
  PostNiAction: TRecordsListNotification;
  PostNiHasData: Boolean;

  procedure DeleteFromSortedList(Index: Integer);
  var
    RecView: TMemRecViewEh;
    i: Integer;
  begin
    RecView := TMemRecViewEh(FSorderViewList[Index]);
    if (RecView.Rec <> MemRec) then
    begin
      for i := 0 to FSorderViewList.Count-1 do
      begin
        if TMemRecViewEh(FSorderViewList[i]).Rec = MemRec then
        begin
          Index := i;
          RecView := TMemRecViewEh(FSorderViewList[Index]);
          Break;
    ...
    if Index < FSorderViewList.Count-1 then
      FSortedListIndexesObsolete := True;
    FSorderViewList.Delete(Index);
    RecView.Free;
{ TODO : Invalidate FInSortedListIndex in List}
  end;

  procedure AddToFilterList(RecView: TMemRecViewEh);
  begin
    if NewIndex >= FMemoryViewList.Count
      then AddRecInMemoryViewList(RecView, FMemoryViewList.Count)
      else AddRecInMemoryViewList(RecView, NewIndex);
  end;

  procedure PostNotifyInfo(MemRec: TMemoryRecordEh;
    Index: Integer; Action: TRecordsListNotification);
  begin
    if PostNiHasData and (PostNiAction <> Action) and (Action <> rlnListChangedEh) then
      raise Exception.Create('PostNotifyInfo already defined');
...
begin
  ...
  case Action of
    ...
    rlnRecChangedEh, rlnRecMarkedForDelEh:
      begin
        ...
        begin
          IdxInSorderList := SearchRec(FSorderViewList, MemRec);
          if IdxInSorderList >= 0 then
            RecView := SortedListItem[IdxInSorderList]
          else
            RecView := nil;
        end;
        ...
        begin
          ...
          PostNotifyInfo(MemRec, NewIndex, Action);
        end else if (OldIndex < 0) and (NewIndex >= 0) then
          PostNotifyInfo(MemRec, NewIndex, rlnRecAddedEh)
        else if (OldIndex >= 0) and (NewIndex < 0) then
          PostNotifyInfo(MemRec, OldIndex, rlnRecDeletedEh);
      end;
    ...
    rlnRecDeletedEh:
      begin
        ...
        begin
          ...
          PostNotifyInfo(MemRec, OldIndex, Action);
        end;
        DeleteFromSortedList(Index);
      end;

    rlnListChangedEh:
      begin
        RefreshFilteredRecsList(False);
        PostNotifyInfo(MemRec, Index, rlnListChangedEh);
      end;
  ...
  if ViewAsTreeList and MemoryTreeList.VisibleItemsObsolete = True then
    Notify(nil, -1, rlnListChangedEh)
  else if PostNiHasData then
    Notify(PostNiMemRec, PostNiIndex, PostNiAction);
end;
...
procedure TRecordsViewEh.RefreshFilteredRecsList(NotifyListChanged: Boolean);
var
  i: Integer;
  RList: TObjectList;
  PrefilterList: TObjectListEh;
begin
  FillSortedList;
  ClearMemoryViewList;
  RList := nil;
  ...
  begin
    ...
    if not Assigned(RList) then
      RList := FSorderViewList;


    PrefilterList := TObjectListEh.Create;
    try

      FilterAborted := False;
      for i := 0 to RList.Count-1 do
      begin
        if FilterRecord(TMemRecViewEh(RList[i]).Rec, i, nil) then
          PrefilterList.Add(RList[i]);
        if FilterAborted then
          Break;
      end;

      if not FilterAborted then
      begin
        for i := 0 to PrefilterList.Count-1 do
          AddRecInMemoryViewList(TMemRecViewEh(PrefilterList[i]), FMemoryViewList.Count);
      end;
    finally
      PrefilterList.Free;
    end;
  ...
  if FSortOrder <> '' then
    ResortList
  else if NotifyListChanged then
    Notify(nil, -1, rlnListChangedEh);
end;

procedure TRecordsViewEh.RefreshFilteredRecsTreeList;
begin
...
procedure TRecordsViewEh.QuickSort(ViewList: TObjectList; L, R: Integer; Compare: TCompareRecords; ParamSort: TObject);
var
  I, J: Integer;
  P: TMemoryRecordEh;
begin
  ...
  repeat
    I := L;
    J := R;
    P := TMemRecViewEh(ViewList[(L + R) shr 1]).Rec;
    repeat
      while Compare(TMemRecViewEh(ViewList[I]).Rec, P, ParamSort) < 0 do
        Inc(I);
      while Compare(TMemRecViewEh(ViewList[J]).Rec, P, ParamSort) > 0 do
        Dec(J);
      if I <= J then
      begin
        ViewList.Exchange(I, J);
        Inc(I);
        Dec(J);
      end;
    until I > J;
    if L < J then
      QuickSort(ViewList, L, J, Compare, ParamSort);
    L := I;
  until I >= R;
end;
...
begin
  QuickSort(FMemoryViewList, 0, FMemoryViewList.Count-1, Compare, ParamSort);

  QuickSort(FSorderViewList, 0, FSorderViewList.Count-1, Compare, ParamSort);
  UpdateSorderViewListIndexes;

  Notify(nil, -1, rlnListChangedEh);
end;

procedure TRecordsViewEh.UpdateSorderViewListIndexes;
var
  i: Integer;
begin
  for i := 0 to FSorderViewList.Count-1 do
    TMemRecViewEh(FSorderViewList[I]).FInSortedListIndex := i;
end;

procedure TRecordsViewEh.FillSortedList;
var
  i: Integer;
begin
  for i := 0 to FSorderViewList.Count-1 do
  begin
    FreeObjectEh(FSorderViewList[I]);
    FSorderViewList[I] := nil;
  end;
  FSorderViewList.Clear;

  if MemTableData <> nil then
  begin
    for i := 0 to MemTableData.RecordsList.Count-1 do
      AddToSortedList(MemTableData.RecordsList[i], i);
  end;
end;

function TRecordsViewEh.AddToSortedList(MemRec: TMemoryRecordEh; Index: Integer): TMemRecViewEh;
begin
  Result := CreateMemRecView(MemRec);
  if Index >= FSorderViewList.Count then
  begin
    FSorderViewList.Add(Result);
    Result.FInSortedListIndex := Index;
  end else
  begin
    FSorderViewList.Insert(Index, Result);
    Result.FInSortedListIndex := Index;
    FSortedListIndexesObsolete := True;
  end;
end;
...
function TRecordsViewEh.CreateMemRecView(Rec: TMemoryRecordEh): TMemRecViewEh;
begin
  Result := TMemRecViewEh.Create;
  Result.Data := Rec;
  Result.FRecordsView := Self;
end;

procedure TRecordsViewEh.AddRecInMemoryViewList({Rec: TMemoryRecordEh;}
  RecView: TMemRecViewEh; Index: Integer);
begin
...
begin
  MemTableData.RecordsList.Rec[GetRec(Index).Index] := Value;
end;
...
function TRecordsViewEh.CalcAggrFieldFunc(const FieldName, AggrFuncName: String): Variant;
type
...
{$ENDIF}
{$IFDEF EH_LIB_10}
    , fldWIDESTRING, fldBLOB, fldDATETIME, fldZSTRING 
{$ENDIF}
{$IFDEF EH_LIB_12}
    , fldINT32, fldINT8, fldUINT8, fldFLOAT,
      fldUnknown, fldUnknown, fldUnknown 
{$ENDIF}
{$IFDEF EH_LIB_13}
    , fldDATETIMEOFFSET, fldUNKNOWN, fldSINGLE 
{$ENDIF}
    );
...
var
  ...
  CurCalcRec: Integer; { Need for store current viewed record number.
                         It's used w ith agregate functions.}
...
{$ELSE}
  {$IFDEF NEXTGEN}
  if CharCount = 0 then
     Result := String.Compare(VarToStr(vs1), VarToStr(vs2), not CaseInsensitive)
  else
    Result := String.Compare(VarToStr(vs1), 0, VarToStr(vs2), 0, CharCount, not CaseInsensitive);
  {$ELSE}
...
type
 ...
 function FilterVarEquals(V1, V2: Variant): Boolean;
 begin
    if VarIsStr(V1) or VarIsStr(V2) then
    begin
      Result := (VarCompareStr(V1, V2, 0, foCaseInsensitive in DataSet.FilterOptions) = 0);
    end
    else if (VarType(V1) = varDate) and (VarType(V2) = varDate) then
    begin
      V1 := TruncDateTimeToSeconds(V1);
      V2 := TruncDateTimeToSeconds(V2);
      Result := (V1 = V2);
    end else
      Result := (V1 = V2);
 end;
...
{$IFDEF EH_LIB_12}
       ftShortint:
        Result := Shortint(PShortint(@Source[0])^);
       ftByte:
        Result := Byte(PByte(@Source[0])^);
{$ENDIF}
{$IFDEF EH_LIB_13}
       ftSingle:
        Result := Single(PSingle(@Source[0])^);
{$ENDIF}
       ftSmallInt:
...
{$ENDIF}
       ...
       begin
         ...
         Result := TruncDateTimeToSeconds(Result);
       end;
...
{$IFDEF EH_LIB_14}
       ftTimeStampOffset:
         VarSQLTimeStampOffsetCreate(Result, TSQLTimeStampOffset(PSQLTimeStampOffset(@Source[0])^));
{$ENDIF}
 ...
 function TimeStampsEqual(const tm1, tm2: TTimeStamp): Boolean;
 begin
   Result := (tm1.Date = tm2.Date) and (tm1.Time = tm2.Time)
 end;
...
begin
  ...
  case NODEClass(NodeType) of
    ...
    begin
      ...
      NodeOp1 := PWordArray(FilterDataStartPos(FilterData))^[0];
    ...
    begin
      ...
      NodeOp2 := PWordArray(FilterDataStartPos(FilterData))^[1];
      ...
      case TCANOperator(NodeOp) of
        coEQ:
          ...
          end else if (VarType(Result_Left) = varDate) and (VarType(Result_Right) = varDate) then
          begin
            Result_Left := TruncDateTimeToSeconds(Result_Left);
            Result_Right := TruncDateTimeToSeconds(Result_Right);
        ...
        coIN:
          try
            ...
            begin
              if FilterVarEquals(Result_Left, ListBuffer[i]) then
              begin
                Result := True;
                Exit;
              end;
    ...
    begin
      ...
      NodeOp3 := PWordArray(FilterDataStartPos(FilterData))^[2];
      
      NodeOp4 := PWordArray(FilterDataStartPos(FilterData))^[3];
        ...
        try
          ...
          begin
            ...
            begin
              NodeOp3 := GetParameters(NodeOp2);
              if NodeOp3 = 0 then
                Result := RecordsView.AccountableItemsCount
              else
                raise Exception.CreateFmt('Incorrect number of arguments in function <%s>', [FuncName]);
            end
...
function TRecordsViewEh.IndexOfSortedListRec(Rec: TMemoryRecordEh): Integer;
var
  i: Integer;
begin
  Result := -1;

  if SortedListItemCount = 0 then
    Exit;

  if SortedListItem[SortedListItemCount-1].Rec = Rec then
    Result := SortedListItemCount-1
  else
  begin
    for i := 0 to SortedListItemCount-1 do
      if SortedListItem[i].Rec = Rec then
      begin
        Result := i;
        Exit;
      end;
...
procedure TRecordsViewEh.ResortList;
begin
  SortData(OnCompareRecords, FOrderByList);
end;

procedure TRecordsViewEh.ResortTreeList;
begin
  if ViewAsTreeList then
...
function TRecordsViewEh.GetSortedListItem(Index: Integer): TMemRecViewEh;
begin
  if ViewAsTreeList then
    Result := MemoryTreeList.FlatListItem[Index]
  else
  begin
    Result := TMemRecViewEh(FSorderViewList[Index]);
  end;
end;

function TRecordsViewEh.GetSortedListItemCount: Integer;
begin
  if ViewAsTreeList
    then Result := MemoryTreeList.FlatListCount
    else Result := MemTableData.RecordsList.Count;
end;

procedure TRecordsViewEh.UpdateSortedListIndexes;
begin
  if ViewAsTreeList then
    MemoryTreeList.UpdateSortedListIndexes
  else
    UpdateSorderViewListIndexes;
  FSortedListIndexesObsolete := False;
end;
...
type
  TCustomMemTableEhCrack = class(TCustomMemTableEh);
...
begin
  RecBuf := TDataSetCrack(FDataSet).AllocRecordBuffer;
  TCustomMemTableEhCrack(FDataSet).RecordToBuffer(Rec, dvvValueEh, RecBuf, -1);
  Result := IsCurRecordBufInFilter(RecBuf);
  TDataSetCrack(FDataSet).FreeRecordBuffer(RecBuf);
end;
{$ELSE}
begin
...
{$IFDEF NEXTGEN}
function TDataSetExprParserEh.IsCurRecordBufInFilter(Buffer: TRecBuf): Boolean;
{$ELSE}
...
{$IFDEF EH_LIB_12}
  WideStringVarType := varUString;
{$ELSE}
  WideStringVarType := varOleStr;
{$ENDIF}
end;
...
begin
  ...
  FFlatListItems := TObjectListEh.Create;
  FRecordsViewEh := ARecordsViewEh;
  FInsertedNodeStack := TObjectListEh.Create;
...
var
  ...
  function SearchNewPos(MemRec: TMemoryRecordEh): Integer;
  var
    CurIndex, AMin, AMax: Integer;
  begin
...
begin
  ...
  Result.FRecordsView := FRecordsViewEh;
  Result.SetIntenalNodeExpanded(DefaultNodeExpanded);
...
var
  ...
  CurMemRec: TMemoryRecordEh;
  MTIndex: TMTIndexEh;
  KeyValue, RefValue: Variant;
begin
  Result := TMemRecViewEh(Root);
  if StartNode = nil then
    StartNode := TMemRecViewEh(Root);

  KeyValue := MemRecord.DataIndexValues[FRecordsViewEh.TreeViewKeyFields, dvvValueEh];
  RefValue := MemRecord.DataIndexValues[FRecordsViewEh.TreeViewRefParentFields, dvvValueEh];
  if VarEquals(KeyValue, RefValue) then
  begin
    Result := TMemRecViewEh(Root);
    Exit;
  end;
  ...
  begin
    ...
    begin
      CurNode := StartNode.NodeItems[I];
      CurMemRec := TMemoryRecordEh(CurNode.Data);
      KeyValue := CurMemRec.DataIndexValues[FRecordsViewEh.TreeViewKeyFields, dvvValueEh];
      RefValue := MemRecord.DataIndexValues[FRecordsViewEh.TreeViewRefParentFields, dvvValueEh];
      if VarEquals(KeyValue, RefValue) and (CurNode <> StartNode) then
      begin
        Result := CurNode;
        Break;
      end
...
begin
  ...
  begin
    ...
    if VarEquals(CurMemRec.DataValues[ParentFieldNames, dvvValueEh],
                 MemRecord.DataValues[KeyFieldNames, dvvValueEh]) and
       (CurMemRec <> MemRecord)
    then
...
begin
  ...
  begin
    ...
    ChildList := TObjectListEh.Create;
...
begin
  ...
  begin
    ...
    if (Index >= 0) and (Index < Result.NodesCount)
...
begin
  ...
  FFlatListItems.Clear;
  CurNode := GetFirst;
  Idx := 0;
  while CurNode <> nil do
  begin
    TMemRecViewEh(CurNode).FInSortedListIndex := Idx;
    FFlatListItems.Add(CurNode);
    CurNode := GetNext(TMemRecViewEh(CurNode));
    Idx := Idx + 1;
  end;

  FRecordsViewEh.FSortedListIndexesObsolete := False;
...
begin
  ...
  if IsUpdateParent and
     (Node.Parent <> nil) and
     (Node.Parent <> Root) and
     (Node.Visible <> Node.NodeParent.NodeVisible)
  then
...
function TMemoryTreeListEh.GetNextVisibleSibling(Node: TMemRecViewEh): TMemRecViewEh;
begin
  Result := TMemRecViewEh(inherited GetNextVisibleSibling(Node));
end;

function TMemoryTreeListEh.GetPriorVisibleSibling(Node: TMemRecViewEh): TMemRecViewEh;
begin
  Result := TMemRecViewEh(inherited GetPrevVisibleSibling(Node));
end;
...
function TMemoryTreeListEh.GetFlatListCount: Integer;
begin
  if FVisibleItemsObsolete then
    BuildVisibleItems;
  Result := FFlatListItems.Count;
end;

function TMemoryTreeListEh.GetFlatListItem(const Index: Integer): TMemRecViewEh;
begin
  if FVisibleItemsObsolete then
    BuildVisibleItems;
  if (Index < 0) or (Index > FFlatListItems.Count-1)
    then Result := nil
    else Result := TMemRecViewEh(FFlatListItems[Index]);
end;

procedure TMemoryTreeListEh.UpdateSortedListIndexes;
begin
...
procedure TMemRecViewEh.SetIntenalNodeExpanded(const Value: Boolean);
begin
  FLockUpdate := True;
  try
    Expanded := Value;
...
procedure TMemRecViewEh.ExpandedChanged;
begin
...
function TMemRecViewEh.GetInSortedListIndex: Integer;
begin
  if FRecordsView.SortedListIndexesObsolete then
    FRecordsView.UpdateSortedListIndexes;
  Result := FInSortedListIndex;
end;
...
procedure TMemoryTreeListOrderByList.AssignFieldIndex(OrderItem: TOrderByItemEh;
  const FieldIndex: Integer);
begin
  OrderItem.MTFieldIndex := FieldIndex;
end;
...
constructor EUniqueKeyViolationEh.Create(const Msg: string; AFieldNames: String; AValue: Variant);
begin
  ...
  case Action of
    rlnRecAddingEh:
      begin
        ...
        begin
          ...
          if KeyFound then
            raise EUniqueKeyViolationEh.Create('Unique key violation ' + sLineBreak +
              ' FieldNames: ' + Fields + sLineBreak +
              ' Value: "' + AnyVarToStrEh(MemRec.DataValues[Fields, dvvValueEh]) + '"',
              Fields, MemRec.DataValues[Fields, dvvValueEh]);
        end;
      end;
    rlnRecAddedEh:
      begin
    ...
    rlnRecChangingEh:
      begin
        ...
        if (Unique or Primary) and (DBVarCompareValue(MemRec.DataValues[Fields, dvvValueEh], FOldValue) <> vrEqual) then
        begin
          KeyFound := FindKeyValueIndex(MemRec.DataValues[Fields, dvvValueEh], NewIndex);
          if KeyFound then
            raise EUniqueKeyViolationEh.Create('Unique key violation ' + sLineBreak +
    ...
    rlnRecDeletedEh, rlnRecMarkedForDelEh:
      begin
        ...
        begin
          FreeObjectEh(FRecList[OldIndex]);
...
procedure TMTIndexEh.SetUnique(const Value: Boolean);
begin
...
procedure TMTIndexEh.CheckUnique;
var
  v: Variant;
  i: Integer;
begin
  ...
  begin
    if not VarIsNull(v) and
      (DBVarCompareValue(v, Item[i].Value) = vrEqual)
    then
      raise EUniqueKeyViolationEh.Create('Unique key violation ' + sLineBreak +
        ' FieldNames: ' + Fields + sLineBreak +
        ' Value: "' + AnyVarToStrEh(v) + '"',
         Fields, v);
    v := Item[i].Value;
  end;
end;
...
procedure TMTIndexesEh.UpdateIndexes;
begin
  RLDataEvent(nil, -1, rlnListChangedEh);
end;
