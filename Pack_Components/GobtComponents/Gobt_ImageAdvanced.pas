...
uses
  Windows, Messages, Classes, Graphics, Controls , SysUtils, EXGDIP, EXGDIPicture,
...
type
  ...
  {$ENDIF}
  TGoImageAdvanced = class(TGraphicControl)
  private
...
procedure TGoImageAdvanced.ButtonChanged(Sender: TObject);
begin
  Invalidate;
end;

procedure TGoImageAdvanced.Click;
begin
  inherited;
end;

procedure TGoImageAdvanced.CMHintShow(var Msg: TMessage);
var
  hi: PHintInfo;
begin
  inherited;
...
procedure TGoImageAdvanced.CMMouseLeave(var Message: TMessage);
begin
...
constructor TGoImageAdvanced.Create(AOwner: TComponent);
begin
  inherited;
...
destructor TGoImageAdvanced.Destroy;
begin
  FButtonClear.Free;
  FButtonSelect.Free;
  FIPicture.Free;
...
procedure TGoImageAdvanced.DoPictureClear;
begin
  if Assigned(OnPictureClear) then
    OnPictureClear(Self);
end;

procedure TGoImageAdvanced.DoPictureSelect(FileName: string; var Allow: boolean);
begin
  if Assigned(OnPictureSelect) then
    OnPictureSelect(Self,FileName, Allow);
end;

procedure TGoImageAdvanced.DoPictureSelected(FileName: string);
begin
  if Assigned(OnPictureSelected) then
    OnPictureSelected(Self,FileName);
end;

procedure TGoImageAdvanced.Loaded;
begin
  inherited;
end;

procedure TGoImageAdvanced.MouseDown(Button: TMouseButton; Shift: TShiftState; X,
  Y: Integer);
begin
  inherited;
...
procedure TGoImageAdvanced.MouseMove(Shift: TShiftState; X, Y: Integer);
begin
  inherited;
...
procedure TGoImageAdvanced.MouseUp(Button: TMouseButton; Shift: TShiftState; X,
  Y: Integer);
begin
  inherited;
...
procedure TGoImageAdvanced.Paint;
var
  h: integer;
  r: TRect;
  s: string;
...
procedure TGoImageAdvanced.PaintTo(ACanvas: TCanvas; DrawRect: TRect);
var
...
procedure TGoImageAdvanced.PictureChange;
begin

end;

procedure TGoImageAdvanced.PictureChanged(sender: TObject);
begin
...
procedure TGoImageAdvanced.SelectPicture;
var
...
procedure TGoImageAdvanced.SetAngle(const Value: Integer);
begin
  FAngle := Value;
  Repaint;
end;

procedure TGoImageAdvanced.SetAutoSizeP(const Value: Boolean);
var
  w,h: double;
begin
  FAutoSize := Value;
...
procedure TGoImageAdvanced.SetGDIPPicture(const Value: TGDIPPicture);
begin
  FIPicture.Assign(Value);
  Invalidate;
end;

procedure TGoImageAdvanced.SetBackgroundColor(const Value: TColor);
begin
  FBackgroundColor := Value;
  FIPicture.BackgroundColor := Value;
end;

procedure TGoImageAdvanced.SetBorderColor(const Value: TColor);
begin
...
procedure TGoImageAdvanced.SetBorderPenStyle(const Value: TPenStyle);
begin
...
procedure TGoImageAdvanced.SetBorderStyle(const Value: TGDIPBorderStyle);
begin
...
procedure TGoImageAdvanced.SetButtonClear(const Value: TPictureButton);
begin
  FButtonClear.Assign(Value);
end;

procedure TGoImageAdvanced.SetButtonSelect(const Value: TPictureButton);
begin
  FButtonSelect.Assign(Value);
end;

procedure TGoImageAdvanced.SetCenter(const Value: Boolean);
begin
...
procedure TGoImageAdvanced.SetCropping(const Value: Boolean);
begin
...
procedure TGoImageAdvanced.SetDoubleBuffered(const Value: Boolean);
begin
  FDoubleBuffered := Value;
  FIPicture.DoubleBuffered := Value;
end;

procedure TGoImageAdvanced.SetEmptyText(const Value: string);
begin
...
procedure TGoImageAdvanced.PictureCleared(Sender: TObject);
begin
  FRefresh := True;
  Repaint;
  FRefresh := False;
...
function TGoImageAdvanced.GetClearRect: TRect;
var
  r: TRect;
begin
  r := ClientRect;
...
function TGoImageAdvanced.GetSelectRect: TRect;
var
  r, br: TRect;
begin
  r := ClientRect;
...
function TGoImageAdvanced.GetVersion: string;
var
  vn: Integer;
begin
  vn := GetVersionNr;
...
function TGoImageAdvanced.GetVersionNr: Integer;
begin
  Result := MakeLong(MakeWord(BLD_VER,REL_VER),MakeWord(MIN_VER,MAJ_VER));
end;

function TGoImageAdvanced.InClearRect(X, Y: integer): boolean;
begin
  Result := PtInRect(GetClearRect,Point(X,Y)) and (ButtonClear.Position <> bpNone);
end;

function TGoImageAdvanced.InSelectRect(X, Y: integer): boolean;
begin
  Result := PtInRect(GetSelectRect,Point(X,Y)) and (ButtonSelect.Position <> bpNone);
end;

procedure TGoImageAdvanced.SetProportional(const Value: Boolean);
begin
...
procedure TGoImageAdvanced.SetStretch(const Value: Boolean);
begin
...
procedure TGoImageAdvanced.SetStretchMode(const Value: TStretchMode);
begin
...
procedure TGoImageAdvanced.SetTransparentBitmap(const Value: boolean);
begin
...
procedure TGoImageAdvanced.SetVersion(const Value: string);
begin

end;

function TGoImageAdvanced.ShouldShowButtonClear: boolean;
begin
...
function TGoImageAdvanced.ShouldShowButtonSelect: boolean;
begin
