...
type
  TGobtProgressModernColorSets = (csOriginal, csBlue, csRed);
  TGobtProgressModernMode = (pmNormal, pmMarquee);
  TMarqueeMode = (mmToLeft, mmToRight);
  TMarqueeSpeed = (msSlow, msMedium, msFast);

  TGoProgressModern = class(TGraphicControl)
  private
    FColorSet: TGobtProgressModernColorSets;
    FProgressDividers: Boolean;
    FBackgroundDividers: Boolean;
    FMarqueeWidth: Longint;
    FMax: Longint;
    FMode: TGobtProgressModernMode;
    FPosition: Longint;
    FShadow: Boolean;
    ...
    Timer: TTimer;
    procedure SetColorSet(newColorSet: TGobtProgressModernColorSets);
    ...
    procedure SetMode(newMode: TGobtProgressModernMode);
  ...
  published
    property Align;
    property ColorSet: TGobtProgressModernColorSets read FColorSet write SetColorSet;
    ...
    property Mode: TGobtProgressModernMode read FMode write SetMode;
...
implementation
uses
  Gobt_ProgressModernColors;


procedure TGoProgressModern.SetColorSet(newColorSet: TGobtProgressModernColorSets);
  begin
    FColorSet := newColorSet;
    Invalidate;
  end;

procedure TGoProgressModern.SetMarqueeWidth(newMarqueeWidth: Integer);
  var
    OldWidth: Longint;
  begin
...
procedure TGoProgressModern.SetProgressDividers(newProgressDividers: Boolean);
  begin
    FProgressDividers := newProgressDividers;
    Invalidate;
  end;

procedure TGoProgressModern.SetBackgroundDividers(newBackgroundDividers: Boolean);
  begin
    FBackgroundDividers := newBackgroundDividers;
    Invalidate;
  end;

procedure TGoProgressModern.SetMax(newMax: Integer);
  begin
    if newMax > 0 then
      FMax := newMax;
...
procedure TGoProgressModern.SetMode(newMode: TGobtProgressModernMode);
  begin
    FMode := newMode;
...
procedure TGoProgressModern.SetPosition(newPosition: Integer);
  begin
...
procedure TGoProgressModern.SetShadow(newShadow: Boolean);
  begin
    FShadow := newShadow;
    if FShadow then
      Height := 19
...
procedure TGoProgressModern.SetSpeed(newSpeed: TMarqueeSpeed);
  begin
    FSpeed := newSpeed;
    case FSpeed of
...
procedure TGoProgressModern.SetStep(newStep: Integer);
  begin
    if (newStep > 0) and (newStep <= (FMax)) then
      FStep := newStep;
  end;

procedure TGoProgressModern.SetVisible(newVisible: Boolean);
  begin
    FVisible := newVisible;
    if FVisible then
      Invalidate
...
procedure TGoProgressModern.MarqueeOnTimer(Sender: TObject);
  begin
    if not (csDesigning in ComponentState) then
      Invalidate;
  end;

procedure TGoProgressModern.PaintNormal;
  var
    POverlay: Longint;
    PJoist: Longint;
    PDistance: Extended;
  ...
  begin
    ...
    with Buffer.Canvas do
      begin
        ...
        Brush.Color := GobtProgressModernColorSets[FColorSet].Progress[0];
        FillRect(Rect(1, 1, POverlay+1, 2));
        Brush.Color := GobtProgressModernColorSets[FColorSet].Progress[1];
        FillRect(Rect(1, 2, POverlay+1, 3));
        Brush.Color := GobtProgressModernColorSets[FColorSet].Progress[2];
        FillRect(Rect(1, 3, POverlay+1, 4));
        Brush.Color := GobtProgressModernColorSets[FColorSet].Progress[3];
        FillRect(Rect(1, 4, POverlay+1, 5));
        Brush.Color := GobtProgressModernColorSets[FColorSet].Progress[4];
        FillRect(Rect(1, 5, POverlay+1, 6));
        Brush.Color := GobtProgressModernColorSets[FColorSet].Progress[5];
        FillRect(Rect(1, 6, POverlay+1, 7));
        Brush.Color := GobtProgressModernColorSets[FColorSet].Progress[6];
        FillRect(Rect(1, 7, POverlay+1, 8));
        Brush.Color := GobtProgressModernColorSets[FColorSet].Progress[7];
        FillRect(Rect(1, 8, POverlay+1, 9));
        Brush.Color := GobtProgressModernColorSets[FColorSet].Progress[8];
        FillRect(Rect(1, 9, POverlay+1, 12));
        Brush.Color := GobtProgressModernColorSets[FColorSet].Progress[9];
        FillRect(Rect(1, 12, POverlay+1, 13));
        Brush.Color := GobtProgressModernColorSets[FColorSet].Progress[10];
        FillRect(Rect(1, 13, POverlay+1, 14));
        Brush.Color := GobtProgressModernColorSets[FColorSet].Progress[11];
        FillRect(Rect(1, 14, POverlay+1, 15));
        Brush.Color := GobtProgressModernColorSets[FColorSet].Progress[12];
        FillRect(Rect(1, 15, POverlay+1, 16));
        Brush.Color := GobtProgressModernColorSets[FColorSet].Progress[13];
            ...
            for i := 1 to PJoist-1 do
              begin
                if Round(PDistance*i)<=POverlay then
                  for k := 0 to 15 do
                      Pixels[Round(PDistance*i), k+1] := GobtProgressModernColorSets[FColorSet].JoistLeft[k];
                if Round(PDistance*i)+1<=POverlay then
                  for k := 0 to 15 do
                      Pixels[Round(PDistance*i)+1, k+1] := GobtProgressModernColorSets[FColorSet].JoistRight[k];
              end;
          end;
      end;
  end;

procedure TGoProgressModern.PaintMarquee;
  var
    PLeft: Longint;
    PRight: Longint;
    PJoist: Longint;
...
procedure TGoProgressModern.Paint;
  var
    PJoist: Longint;
    PDistance: Extended;
    i: Longint;
...
procedure TGoProgressModern.SetBounds(ALeft: Integer; ATop: Integer; AWidth: Integer; AHeight: Integer);
  begin
    if AWidth < 100 then
      AWidth := 100;
    if FShadow then
...
procedure TGoProgressModern.StepIt;
  begin
    if FMode = pmNormal then
      begin
...
constructor TGoProgressModern.Create(AOwner: TComponent);
  begin
...
destructor TGoProgressModern.Destroy;
  begin
    Timer.Free;
    Buffer.Free;
    inherited;
