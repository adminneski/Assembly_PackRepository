unit GOXFileDownloadThread;

interface

uses
  Windows, Classes, SysUtils, GOXFileDownUtils, WinInet;
...
type
  ...
  TGOXThreadDownload = class(TThread)
  private
    FURL: String;
    FMethod: THTTPMethod;
    FPostData: TStrings;
...
procedure TGOXThreadDownload.Execute;
var
  AStream: TStream;
...
constructor TGOXThreadDownload.Create(const URL, FileName, PartExt:string);
begin
...
procedure TGOXThreadDownload.Stop;
begin
  FCanceled := True;
end;

procedure TGOXThreadDownload.DoStart;
begin
  if Assigned(FOnStart) then
    FOnStart(Self);
end;

procedure TGOXThreadDownload.DoProgress;
begin
  if Assigned(FOnProgress) then
    FOnProgress(Self, FDownloadedFileSize, FCalculatedFileSize);
end;

procedure TGOXThreadDownload.DoFinish;
begin
  if Assigned(FOnFinish) then
    FOnFinish(Self, FDownloadState, FCanceled);
end;

procedure TGOXThreadDownload.SetPostData(Value: TStrings);
begin
  FPostData.Assign(Value);
end;

destructor TGOXThreadDownload.Destroy;
begin
  FPostData.Free;
  inherited;
end;
