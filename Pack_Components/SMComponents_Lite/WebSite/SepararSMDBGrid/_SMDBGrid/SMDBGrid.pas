...
{ Copyright (C) 1998-2017, written by Mike Shkolnik, Scalabium Software
...
procedure WriteTitleText(ACanvas: TCanvas; ARect: TRect; DX, DY: Integer;
  const Text: TSMGString; Alignment: TAlignment; TextEllipsis: TSMTextEllipsis; IsWordBreak: Boolean {$IFDEF SMForDelphi4} ; ARightToLeft: Boolean {$ENDIF});
const
...
{$ENDIF}
    begin
      ...
      {$IFDEF USE_TNT}
      Tnt_DrawTextW(ACanvas.Handle, PWideChar(Text), Length(Text), ARect,
...
{$ENDIF}
      ...
      {$IFDEF USE_TNT}
        Tnt_DrawTextW(Handle, PWideChar(Text), Length(Text), R,
          EllipsisFlags[TextEllipsis] or AlignFlags[Alignment] {$IFDEF SMForDelphi4} or RTL[ARightToLeft] {$ENDIF} );
      {$ELSE}
...
begin
  ...
  try
    ...
    if ((VarType(Value) = varBoolean) and VarAsType(Value, varBoolean)) then
      CheckState := cbChecked
  except
  end;
          ...
          begin
            ...
            begin
              ...
              try
                ...
                if VarAsType(DrawColumn.Field.Value, varBoolean) then
                  CheckState := cbChecked
              except
              end;
              Canvas.FillRect(ARect);
...
//          intGroupIndex := i;
          ...
          begin
            ...
            WriteTitleText(Canvas, RectLookup, -2 + Ord(gdFixed in State), 2, str, al,
                           teNone,
                           eoTitleWordWrap in ExOptions
                           {$IFDEF SMForDelphi4},
                           (BiDiMode <> bdLeftToRight)
...
//       (Assigned(Field) and HighlightCell(DataCol, DataLink.ActiveRecord, Field.AsString, State)) then
    ...
    begin
      ...
      Result := {Column.Color;// }Canvas.Brush.Color;
    end
  end;
...
begin
  ...
  if DefaultDrawing and Highlight and
...
//20051027     (GridStyle.Style <> gsNormal) and
  ...
  begin
    ...
    begin
      Canvas.FillRect(Rect);
      WriteTitleText(Canvas, Rect, 2, 2, Column.Field.AsString, Column.Alignment,
  ...
  begin
    ...
    try
      ...
      if VarAsType(Column.Field.Value, varBoolean) then
{      if ((Column.Field.DataType = ftBoolean) and Column.Field.Value) or
         ((Column.Field is TNumericField) and Column.Field.Value) or
         ((Column.Field is TStringField) and Column.Field.AsString = 'True') or
       then
}
        CheckState := cbChecked
    except
      ...
      begin
        ...
        begin
          if Assigned(Field) then
            ...
            Value := '';
          WriteTitleText(Canvas, Rect, 2, 2, Value, Column.Alignment,
                         GetColumnEllipsis(Column),
                         (eoCellWordWrap in ExOptions) //False
                         {$IFDEF SMForDelphi4},
                         (BiDiMode <> bdLeftToRight)
